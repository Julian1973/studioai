...
def gate3_dryrun(pkg_path, code, episode="Ep1"):
    ...
    return {"builder": prep["builder"], "format": prep.get("format", "?"), "beat": code, "refuse": prep["refuse"], "reason": prep["reason"],
            ...
            "validator_ok": prep["report"]["ok"], "rejects": prep["report"]["rejects"],
            "length": prep["report"].get("length"), "prompt": prep["prompt"]}
...
def _prev_clip_path(beats, i, episode):
    """The previous non-vision beat's already-rendered clip in this scene (for the continuity-tail handoff), or None.
    `beats` = the scene's beats in order; `i` = the current beat's index. Used to chain each take off the last."""
    _vis = getattr(P, "vision_for", None)
    j = i - 1
    while j >= 0:
        pb = beats[j]; pc = pb.get("beatCode") or pb.get("shotCode")
        if not (_vis and _vis(episode, str(pc))):           # skip a VISION beat as a continuity source
            ps = pb.get("slug") or (pc or "").replace(".", "_")
            pclip = f"media/{episode}_{pc}_{ps}.mp4"
            return pclip if os.path.exists(pclip) else None
        j -= 1
    return None
    ...
    for _i, b in enumerate(beats):
        ...
        if os.environ.get("CB_REUSE_VOICE") and os.path.exists(_vo):
            ...
            try:
                ...
                print(f"  {code}: voice track failed ({str(e)[:120]}) — take falls back to native voice", flush=True)
        ...
        imgs = [start] + [a for c in b.get("characters", []) if (a := _anchor(c))]
        # CONTINUITY TAIL (Gate-3 FLOW) — chain off the previous beat's clip LAST FRAME so the take flows on (no jump-cut)
        # and carries forward pose/position/state (e.g. the pollen moustache). Appended as the FINAL reference image; the
        # prompt's continuity_video instruction points at it. CB_CONTINUITY_TAIL=0 disables. Fail-open (skip on any error).
        if not raw and os.environ.get("CB_CONTINUITY_TAIL", "1") not in ("", "0", "false", "False", "no"):
            _pc = _prev_clip_path(beats, _i, episode)
            if _pc:
                try:
                    _cf = f"media/_cont_{code}.png"; cb_gen.last_frame(_pc, out=_cf)
                    if os.path.exists(_cf):
                        imgs = imgs + [_cf]
                        if isinstance(prompt, dict):
                            prompt["continuity_video"] = {"previous_clip_tail": "the FINAL supplied reference image",
                                "instruction": ("The FINAL supplied reference image is the EXACT last frame of the previous "
                                    "beat. Begin this take from that precise state — same character pose, position, facing, "
                                    "lighting and any carried props (e.g. a pollen moustache) — and flow CONTINUOUSLY from it "
                                    "with no hard cut, reset or teleport.")}
                        print(f"  {code}: continuity tail ← {os.path.basename(_pc)} (flows from its last frame)", flush=True)
                except Exception as _e:
                    print(f"  {code}: continuity tail skipped ({str(_e)[:80]})", flush=True)
