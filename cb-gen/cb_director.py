...
CANON = ROOT / "CRYSTAL_BEARS_LOCKED_CANON.md"
CHARS = HERE / "config" / "characters.json"
...
def enforce_verbatim(beats, scene_dialogue, scene_num, log=print):
    ...
    slots = []
    for b in sorted(beats, key=lambda x: x.get("beatCode") or ""):
    ...
    # `dropped` lets Gate 1 flag the scene for a re-break rather than sign off an incomplete scene.
    ...
    if dropped:
        ...
        log(f"      ⚠ VERBATIM GATE: scene {scene_num} has {len(dups)} DUPLICATED line(s) — re-break to remove repeats.", flush=True)
    else:
...
def direct(script_path, episode, title, log=print):
    ...
    _pmap = {}
    try:
        for _ps in cb_script.parse(script, _script_roster()):
        ...
        for sc in scenes:
            ...
            def _draft_and_gate(retry_note=""):
                ...
                if _scene_dialogue:                          # HARD VERBATIM GATE — snap every line back to the writer's EXACT words
                    _beats, _drop = enforce_verbatim(_beats, _scene_dialogue, sc["sceneNumber"], log=log)
                return _beats, _drop

            def _missing_tuples(dropped_lines):
                dn = {_norm_line(d) for d in dropped_lines}
                return [(c, l, p) for (c, l, p) in _scene_dialogue if _norm_line(l) in dn]

            beats, dropped = _draft_and_gate()
            log(f"      first draft: {len(beats)} beats", flush=True)
            # COMPLETENESS RETRY — the LLM's most common failure is dropping a scene's LAST line/button. Retry the WHOLE
            # scene breakdown (bounded, twice) with the exact missing line(s) named, before falling back to a mechanical
            # guarantee. This makes "every script line appears" an ACTUAL guarantee, not just a loud warning.
            _retry_n = 0
            while dropped and _retry_n < 2:
                _retry_n += 1
                miss = _missing_tuples(dropped)
                note = ("CRITICAL FIX — your PREVIOUS breakdown of this scene DROPPED the following line(s), which MUST "
                        "appear EXACTLY as a dialogue cut (often the scene's LAST line/button, easy to lose at the tail): "
                        + "; ".join(f'{c}: "{l}"' for c, l, _p in miss)
                        + ". Re-do the FULL beat breakdown for this scene — keep every other line exactly as before, and "
                        "make sure this one is included too. Do not drop it again.")
                log(f"      ↻ retry {_retry_n}/2 — re-breaking scene {sc['sceneNumber']} to recover {len(miss)} dropped line(s)...", flush=True)
                beats, dropped = _draft_and_gate(retry_note=note)
            if dropped:
    ...
    for _sn, _bs in _bysc.items():
        _bs = sorted(_bs, key=lambda x: (x.get("beatCode") or ""))
    ...
    C = {"_note": C.get("_note", ""), episode: bm.get("continuity", {})}
    ...
    arc = {"_note": A.get("_note", ""), **arc}
    json.dump(arc, open(arcp, "w"), indent=1, ensure_ascii=False)
