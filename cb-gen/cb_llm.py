...
ENVIRONMENT only (cb-gen/.env) — never hardcoded in source, never sent to app.html / the frontend.
...
import cb_gen   # importing cb_gen loads cb-gen/.env into os.environ (keys never leave the backend)
...
def _openai_key():
    ...
    if not key:
        raise SystemExit("OPENAI_API_KEY is missing — add it to cb-gen/.env (env only; never hardcode it or expose "
