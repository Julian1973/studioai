...
def _shotord(scene, code):
    try: return (int(scene), [int(x) for x in str(code).split(".")])
...
def build_keyframe_prompt(shot, sc, master_path=None, note="", episode="Ep1", chain_ref=None, end_from=None, end_beat=False):
    ...
    constraints = ("CONSTRAINTS: Avoid multiple faces, distorted proportions, or duplicating the characters. "
                   f"Exactly {one_each}.{size_clause}{sig}{neg}{wings}{clean_base}{env_neg}")
...
def seedance_json(beat, style=None, episode="Ep1", audio_dur=None):
    ...
    DEFAULT_STYLE = ("Premium 3D CGI, Disney/Pixar animation style, 8K resolution, Octane render, subsurface "
                     "scattering (fur/skin), volumetric lighting, cinematic depth of field, hyper-realistic textures.")
    def _strip_cuts(s):                                  # strip editorial / camera-cut language so ONE take remains
        s = re.sub(r"\b(shot\s*\d+|cut to|counter[- ]?shots?|reaction holds?|hook close-?ups?|"
                   r"whip[- ]?pan(?:ning)?(?:\s+(?:following|on|to)\s+[\w'’]+)?|quick cut|hard cut|montage)\b",
                   "", s or "", flags=re.I)
        return re.sub(r"[ ]{2,}", " ", s).strip(" ,;.-")
    def _clean_line(said):                               # spoken text -> sentence case (NO shouty caps -> no subtitle hallucination)
        said = re.sub(r"\[[^\]]*\]", "", said or "")     # drop V3 [delivery tags]
        said = re.sub(r"\s{2,}", " ", said).strip()
        said = re.sub(r"\b[A-Z][A-Z'’]{1,}\b", lambda m: m.group(0).lower(), said)   # flatten SHOUTY caps (emphasis lives in @Audio1)
        said = re.sub(r"(^|[.!?]\s+)([a-z])", lambda m: m.group(1) + m.group(2).upper(), said)  # restore sentence-initial caps
        return said
    raw_cuts = beat.get("cuts") or []
    chars = [c for c in (beat.get("characters") or []) if c]
    dur = int(beat.get("durationSec") or 11); dur = max(8, min(15, dur))

    # parse each raw cut -> {action, lines:[(name, said), ...]}  (a cut may hold MULTIPLE speakers)
    segs = []
    for c in raw_cuts:
        action = _strip_cuts(c.get("action") or "")
        lines = []
        for ln in re.split(r"[\r\n]+", (c.get("dialogue") or "").strip()):
            ln = ln.strip()
            if not ln:
                continue
            m = re.match(r"\s*([A-Z][A-Z'’()., ]{1,40}?)\s*:\s*(.+)$", ln)
            name, said = (m.group(1), m.group(2)) if m else ("", ln)
            name = re.sub(r"\s*\([^)]*\)", "", name).strip().title()   # "ALL (AIDA, HOWEY, MISTY, LUNA)" -> "All"
            said = _clean_line(said)
            if said:
                lines.append((name, said))
        segs.append({"action": action, "lines": lines, "vt": (c.get("voiceTreatment") or "").strip()})

    # ── BEAT POSITION → INTER-BEAT CONTINUITY (the cure for the jumpiness): a beat after the first FLOWS ON from the prior beat.
    code = str(beat.get("beatCode") or beat.get("shotCode") or "")
    bn = (code.split(".")[-1].lstrip("Bb") or code) if code else ""
    try: bnum = int(re.sub(r"\D", "", bn) or "1")
    except Exception: bnum = 1
    is_first = bnum <= 1
    prev = bnum - 1
    speakers = [s for s in (beat.get("speakers") or []) if s] or sorted({n for s in segs for n, _ in s["lines"] if n})
    nonspeakers = [c for c in chars if c not in speakers]

    # location / env
    try: sc = scene_cfg(episode, str(beat.get("sceneNumber") or ""))
    except Exception: sc = {}
    world = (sc.get("location") or sc.get("name") or "").strip()
    env = world.split(",")[0].strip().rstrip(".")
    env = (env[0].upper() + env[1:]) if env else ""

    # (1) CAMERA — ONE continuous take, no cuts; a non-first beat flows ON from the previous beat's final composition
    cam = _strip_cuts(beat.get("cameraArc") or (raw_cuts[0].get("framing") if raw_cuts else "")) or "Medium two-shot, locked and stable"
    if len(chars) >= 2 and not re.search(r"two[- ]?shot|wide|establish|group", cam, re.I):
        cam += ", framing all characters in one frame"
    cont_cam = "" if is_first else f"Continue directly from Beat {prev}'s final locked composition — the camera and framing flow on from it with NO reset, cut or jump. "
    camera = cont_cam + cam.rstrip(". ") + ". ONE continuous unbroken take with no cuts."

    # (2) VISUAL PROMPT — environment + (continuity) + GEOGRAPHIC blocking (the cut actions) + weighted physics
    acts = [(a if a[-1:] in ".!?" else a + ".") for a in (s["action"].strip() for s in segs if s["action"].strip())]
    flow = " ".join(acts).strip() or _strip_cuts(beat.get("startState") or beat.get("storyBeat") or "")
    cont_v = "" if is_first else f"Continuing directly from Beat {prev}. "
    visual_prompt = re.sub(r"[ ]{2,}", " ", ((env + ". ") if env else "") + cont_v + flow).strip()
    visual_prompt += " Heavy, weighted physics throughout — real momentum, squash-and-stretch and follow-through, never floaty."

    # (3) STYLE — match the provided keyframe (and the previous beat) EXACTLY, so the look stays continuous beat-to-beat
    style_out = style or ("Premium 3D CGI Disney/Pixar-style feature animation matching the provided keyframe"
                          + ("" if is_first else f" and Beat {prev}") + " exactly. Expressive physical comedy, cinematic "
                          + (f"{env.lower()} " if env else "") + "lighting, volumetric atmosphere, detailed materials and "
                          "cinematic depth of field.")

    # (4) TIMELINE — action + 'X lip-syncs exactly to the supplied @Audio1 line: "..."' across the @Audio1 length, then a HOLD
    ad = float(audio_dur or 0)
    active = min(round(ad, 1) if ad > 0 else round(dur * 0.8, 1), round(dur - 0.5, 1))
    content = [s for s in segs if s["action"] or s["lines"]]
    weights = [max(1, len(s["action"]) + sum(len(t) for _, t in s["lines"]) * 2) for s in content] or [1]
    tot = sum(weights); parts = []; t = 0.0
    for i, s in enumerate(content):
        t1 = active if i == len(content) - 1 else round(t + active * weights[i] / tot, 1)
        bits = []
        if s["action"]:
            bits.append(s["action"].rstrip("."))
        vt = s.get("vt")
        for name, said in s["lines"]:
            if vt == "group_chorus":
                bits.append(f'the group speaks "{said}" in UNISON — broad, general group mouth movement on all of them; '
                            f'do NOT lip-sync this line to any single character')
            elif vt == "underwater_vo":
                bits.append(f'{name or "Keen"} is UNDERWATER — treat "{said}" as internal/muffled voice-over; do NOT '
                            f'attempt precise visible underwater mouth lip-sync; convey it through eyes, body strain, '
                            f'bubbles, the pulling action and the wristband glow')
            else:
                bits.append((f'{name} lip-syncs exactly to the supplied @Audio1 line: "{said}"') if name
                            else (f'a voice lip-syncs exactly to the supplied @Audio1 line: "{said}"'))
        parts.append(f"{t}-{t1}s: " + ". ".join(bits) + ".")
        t = t1
    parts.append(f"{active}-{dur}s: HOLD the final composition for a silent comedy reaction beat — settle into the final "
                 f"pose and hold still (any hovering character keeps its wings beating); no new action and no new dialogue.")
    timeline = " ".join(parts)

    # (5) AUDIO — @Audio1 voice UNCHANGED; Seedance generates the named music + ambience + SFX, with continuity + ducking
    sfx = (beat.get("sfxIntent") or beat.get("soundDesign") or beat.get("soundIntent") or "").strip()
    amb = (f"{env.lower()} ambience" if env else "natural environmental ambience")
    spk_join = (", ".join(speakers[:-1]) + " and " + speakers[-1]) if len(speakers) > 1 else (speakers[0] if speakers else "")
    # MUSIC = a NAMED genre/mood, kept DISTINCT from the SFX (Director's musicCue/scoreCue wins; else comedyMode genre).
    _mc = str(beat.get("comedyMode") or "").upper()
    music = ((beat.get("musicCue") or beat.get("scoreCue") or "").strip()
             or ("Playful, bouncy cinematic comedy underscore — light pizzicato strings, ukulele, woodwinds, cartoon stings." if _mc == "BIG"
                 else "Tender, warm orchestral underscore — soft strings, light piano, gentle and emotional." if _mc == "TRUE"
                 else "Warm, gentle orchestral underscore — soft strings, light woodwinds, harp and warm pads."))
    sfx_prompt = (
        f"Use @Audio1 UNCHANGED as the final ElevenLabs V3 voice performance{'s' if len(speakers) != 1 else ''}"
        + (f" for {spk_join}" if spk_join else "") + ". Generate ALL other audio. "
        + f"Music: {music} "
        + (f"Maintain musical and environmental continuity with Beat {prev} — carry its underscore and ambience straight on, evolving with this beat. " if not is_first else "")
        + f"Ambience: {amb}, plus subtle natural character movement (soft wing-buzz for any hovering bee — manic for Fuzzby, calm and controlled for Zenny when present). "
        + (f"Sound effects — add these in-world, tightly timed to the on-screen action, soft and cartoon-warm (gentle for ages 4-8): {sfx} " if sfx else "")
        + "Immediately before each supplied dialogue line, sharply duck the music, ambience and effects so the supplied voice lands cleanly and clearly, then resume after the line. "
        + f"During the final hold, keep only very subtle {amb}."
    )

    # (6) NEGATIVE — anti-cut/-drift/-text/-revoice + per-character lip-sync lock + keep-exactly + per-character SSOT avoids
    negative = ("No camera cuts, transitions or angle changes. No morphing, flickering, character drift, duplicated "
                "characters or off-model faces. No text overlays or subtitles. No extra, added, duplicated or reassigned "
                "dialogue. Do not generate, replace, imitate, alter or retime speech; the only voices are the supplied "
                "@Audio1. Lip-sync each line ONLY to its named character"
                + (f"; {', '.join(nonspeakers)} never speak" if nonspeakers else "") + ". No Chinese, Mandarin or "
                "non-English speech. No onomatopoeia or sound-effect words spoken aloud or shown as text. Keep every "
                "character EXACTLY as in the provided keyframe and add nothing to them. Any hovering character keeps its "
                "wings beating naturally even when the body holds still — do not freeze the wings.")
    # IDENTITY LOCK — carry each character's SSOT `avoid` (characters.json, same source the keyframe uses) into the CLIP negative.
    avoids = [f"{c} wears NO {str(CHARACTERS[c]['avoid']).rstrip('.')}" for c in chars if CHARACTERS.get(c, {}).get("avoid")]
    if avoids:
        negative += " " + "; ".join(avoids) + "."
    # PRODUCTION VOICE TREATMENTS — group chorus + underwater VO (set per cut in the beat package)
    vts = {s.get("vt") for s in segs}
    if "group_chorus" in vts:
        negative += (" A line marked as a group/chorus is spoken in UNISON by the group with broad general mouth "
                     "movement — never lip-sync a chorus line to one single character.")
    if "underwater_vo" in vts:
        negative += (" Underwater lines are internal/muffled voice-over — no precise underwater mouth lip-sync; "
                     "convey them through eyes, body strain, bubbles and the wristband glow.")

    return {
        "scene_id": f"{episode}_Sc{beat.get('sceneNumber','')}_Beat{bn}",
        "camera": camera,
        "visual_prompt": visual_prompt,
        "style": style_out,
        "timeline": timeline,
        "audio": {"audio_urls": ["@Audio1"], "generate_audio": True, "sfx_prompt": sfx_prompt},
        "constraints": {"negative_prompt": negative},
    }
