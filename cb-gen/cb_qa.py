...
def check_clip(shot, clip, keyframe=None, anchors=None, sc=None, episode="Ep1", prompt=None, next_beat=None):
    ...
    try:
        ...
        if en is not None and (big or has_action) and max(en) < FROZEN_EPS:
        ...
        for c, n in seen.items():
            if c in CLIP_CORROBORATE and n < 2: notes.append(c)   # a lone single-frame hit → advisory only
            else: _add(c)
        # Pass B: ONE multi-frame continuity + weight call, keyframe as the truth
