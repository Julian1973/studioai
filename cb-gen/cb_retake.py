#!/usr/bin/env python3
"""GATE 5 — RETAKES (surgical). Regenerate ONE shot inside a beat (continuity-matched) and splice it back at its
...
def director_retake_brief(pkg, code, sh, beat, char_refs, issue, change, episode="Ep1"):
    ...
    cache = f"media/_retake_brief_{code}_s{sh.get('index')}.json"
...
def regen_shot(pkg, ref, change, episode="Ep1", render_dur=5, out=None, preserve_length=True, issue=""):
    ...
    before = _make_review_clip(beat_clip, fs, fe, cref, sin, sout, "BEFORE",
                               f"media/_retake_{code}_s{shotnum}_before.mp4", fps)
    d = json.load(open(pkg))
    ...
    seed = f"media/_retake_seed_{code}_s{shotnum}.png"
    ...
    repl = f"_retake_{code}_s{shotnum}.mp4"
    print(f"  GATE 5 retake {ref}: rendering replacement shot ({render_dur}s, seeded) — action: {action[:80]}…", flush=True)
    try:
    ...
    graded = _match_grade(replacement, seed, f"media/_retake_{code}_s{shotnum}_graded.mp4")
    seam = _seam_delta(seed, graded)
    print(f"  GATE 5 retake {ref}: grade-matched={graded != replacement} · seam Δ={seam} (≈0 = lighting lines up at the cut)", flush=True)
    ...
    after = _make_review_clip(out, fs, fe, cref, sin, sout, "AFTER · retaken",
                              f"media/_retake_{code}_s{shotnum}_after.mp4", fps)
