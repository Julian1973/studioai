...
def compact_seedance_prompt(a):
    ...
    # 11. continuity_out
    # 12. continuity_video — the temporal reference to the previous beat's clip tail (Gate-3 continuity layer)
    cv = a.get("continuity_video") or {}
    continuity_video = {"previous_clip_tail": cv.get("previous_clip_tail_ref", ""),
                        "tail_duration": cv.get("previous_clip_tail_duration", 5),
                        "instruction": cv.get("continuity_video_instruction", "")}
    return {"references": references, "subject": subject, "scene": scene, "action_timeline": action_timeline,
            ...
            "continuity_out": _cap(a.get("continuity_out", ""), 240), "continuity_video": continuity_video}

_COMPACT_FIELDS = ("references", "subject", "scene", "action_timeline", "camera", "performance",
                   "physical_staging", "audio", "style", "negative", "continuity_out", "continuity_video")
...
# ── GATE-3 CONTINUITY VIDEO REFERENCE LAYER ──────────────────────────────────────────────────────────────────────
# Lesson learned: prompting ISOLATED shots then editing them together causes continuity drift. Instead each non-first
# beat continues from the LAST ~3-5s of the PREVIOUS APPROVED clip (a temporal video reference) + the character/scene
# refs. This layer SELECTS that tail + VALIDATES it; it does NOT extract video or render (support + validation only).
CONTINUITY_VIDEO_INSTRUCTION = (
    "Continue directly from the previous approved clip tail. Preserve character positions, lighting, camera direction, "
    "motion momentum, emotional state, pollen/crystal/wristband state and continuity_out from the previous beat.")
# a beat that CARRIES one of these visual states from the previous beat HARD-FAILS without the previous tail.
# Curated + word-boundary matched: NO "pollen"/"crystal" (ambient world-detail in every beat) and NO "rain"
# (matches "rainforest"). Keen's wristband/crystal-SET change is detected from the keenWristbands field, not text.
_CONTINUITY_CARRY_TERMS = ("moustache", "goatee", "smear", "wristband", "aquamarine",
                           "storm", "thunder", "lightning", "downpour", "soaked", "drenched", "wet", "muddy")
...
def _clip_approved(episode, scene, code):
    """True if locked.json marks this beat's CLIP approved (the per-beat 'clip' lock)."""
    lp = os.path.join(os.path.dirname(os.path.abspath(__file__)), "locked.json")
    ...
    try:
        ...
        return False
    return bool((((d.get(episode, {}) or {}).get(str(scene), {}) or {}).get("beats", {}) or {}).get(code, {}).get("clip"))

def continuity_video_ref(pkg_path, beat_code, episode="Ep1", tail_seconds=5):
    """Gate-3 Continuity Video Reference. The last ~3-5s of the PREVIOUS APPROVED clip in the SAME scene becomes a
    temporal video reference so Seedance continues motion/lighting/state instead of restarting from a still keyframe.
    SUPPORT + VALIDATION ONLY — selects the tail + validates; never extracts video or renders. tail clamps to 3-5 (5)."""
    tail_seconds = max(3, min(5, int(tail_seconds or 5)))
    d = json.load(open(pkg_path)); here = os.path.dirname(os.path.abspath(__file__))
    beats = d.get("beats") or d.get("shots") or []
    beat = next((b for b in beats if (b.get("beatCode") or b.get("shotCode")) == beat_code), None)
    r = {"previous_clip_tail_ref": "", "previous_clip_tail_duration": tail_seconds,
         "continuity_video_instruction": "", "continuity_video_required": False,
         "continuity_video_status": "unknown", "previous_beat": None, "rejects": [], "warnings": []}
    if not beat:
        r["continuity_video_status"] = "beat_not_found"; return r
    scene = str(beat.get("sceneNumber"))
    sb = [b for b in beats if str(b.get("sceneNumber")) == scene]
    idx = next((i for i, b in enumerate(sb) if (b.get("beatCode") or b.get("shotCode")) == beat_code), None)
    prev = None; j = (idx - 1) if idx is not None else -1
    while j >= 0:                                                   # the previous NON-VISION beat in this scene
        cc = sb[j].get("beatCode") or sb[j].get("shotCode")
        if not P.vision_for(episode, str(cc)):
            prev = sb[j]; break
        j -= 1
    # does THIS beat CARRY a visual state from the previous beat? (moustache / wristband / crystal / storm …)
    blob = (str(beat.get("startState", "")) + " " + str((beat.get("continuity") or {}).get("opensFrom", "")) + " "
            + str(beat.get("continuity_in", "")) + " " + str(beat.get("keenWristbands", ""))).lower()
    carries = sorted({k for k in _CONTINUITY_CARRY_TERMS if re.search(r"\b" + re.escape(k) + r"\b", blob)})
    if prev is None:                                               # first beat of the scene — no tail required
        r["continuity_video_status"] = "first_beat_no_tail_required"; return r
    pcode = prev.get("beatCode") or prev.get("shotCode"); r["previous_beat"] = pcode
    if beat.get("keenWristbands") and prev.get("keenWristbands") and beat.get("keenWristbands") != prev.get("keenWristbands"):
        carries = sorted(set(carries) | {"wristband-state-change"})   # Keen's wristband/crystal-set changed → needs the prior state
    r["continuity_video_required"] = bool(carries)
    r["continuity_video_instruction"] = CONTINUITY_VIDEO_INSTRUCTION
    pslug = prev.get("slug") or pcode.replace(".", "_")
    pclip = f"media/{episode}_{pcode}_{pslug}.mp4"
    exists = os.path.exists(os.path.join(here, pclip))
    approved = _clip_approved(episode, scene, pcode)
    indep = bool(beat.get("INDEPENDENT_START") or beat.get("independent_start"))
    ov = str(beat.get("continuity_tail_override") or "").strip()   # a beat may pin its tail; validate the right scene/beat
    if ov and pcode not in ov:
        r["continuity_video_status"] = "override_wrong_beat"
        r["rejects"].append(f"CONTINUITY HARD FAIL: continuity_tail_override {ov!r} is not the expected previous beat {pcode} (wrong scene/beat).")
        return r
    if not exists:
        r["continuity_video_status"] = "previous_clip_missing"
        if carries:
            r["rejects"].append(f"CONTINUITY HARD FAIL: {beat_code} carries {carries} from {pcode}, but {pcode}'s clip is not rendered — render & approve {pcode} first to supply the tail.")
        elif not indep:
            r["warnings"].append(f"continuity: {beat_code} has no previous-clip tail ({pcode} not rendered yet) — mark INDEPENDENT_START or confirm no tail before rendering.")
        return r
    if not approved and not os.environ.get("CB_ALLOW_UNAPPROVED_TAIL"):
        r["continuity_video_status"] = "previous_clip_not_approved"
        r["rejects"].append(f"CONTINUITY HARD FAIL: {beat_code}'s previous clip {pcode} exists but is NOT an approved clip (set CB_ALLOW_UNAPPROVED_TAIL=1 to override).")
        return r
    r["previous_clip_tail_ref"] = f"{pclip}#last{tail_seconds}s"
    r["continuity_video_status"] = "tail_available"
    return r
...
def build_for_beat(pkg_path, beat_code, episode="Ep1"):
    ...
    authoring["continuity_video"] = continuity_video_ref(pkg_path, beat_code, episode)   # Gate-3 temporal continuity
...
def get_seedance_prompt(pkg_path, beat_code, mode="render", episode="Ep1"):
    ...
    # GATE-3 CONTINUITY — a carried-state beat with no previous-clip tail HARD-FAILS; a non-carrying one only warns.
    cont = a.get("continuity_video") or {}
    cont_rejects = cont.get("rejects", []); cont_warnings = cont.get("warnings", [])
    if not auth_ok:
        ...
        status = "NEEDS_PREVIOUS_TAIL"                 # carries a visual state but the previous-clip tail isn't available/approved
    ...
    if _def:
        gen_status = ("NEEDS_SOURCE_DATA_FIX" if not auth_ok
                      ...
                      else "NEEDS_PREVIOUS_TAIL" if cont_rejects
        ...
        return {"builder": "cb_segprompt (GENERATED 6-section)", "mode": mode, "format": "DEFINITIVE_PROSE",
                ...
                "keyframe_exists": kf_exists, "continuity_video": cont,
                ...
                "hard_fails": r["report"]["rejects"] + cont_rejects, "warnings": cont_warnings}
    return {"builder": "cb_seedance", "mode": mode, "format": fmt_label, "prompt": chosen,
            ...
            "hard_fails": r["report"]["rejects"] + r["compact_report"]["rejects"] + cont_rejects,
            "warnings": chosen_rep["warnings"] + cont_warnings}
