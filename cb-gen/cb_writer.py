...
ROOT = HERE.parent
SCRIPTS = ROOT / "cb-studio" / "data" / "scripts"
...
def write(seed, episode="Ep1", log=print):
    ...
    json.dump({
        ...
        "scorecard": sc, "seed": seed, "model": WRITER_MODEL,
    ...
    log(f"  ✓ wrote {script_path.name} (LOCKED screenplay) + {sidecar.name} (scorecard) → ready for Gate 1", flush=True)
    return {"script": script_path.name, "scoreFile": sidecar.name, "title": title,
            "scenes": len(d.get("scenes", [])), "scorecard": sc}
