...
BASE=ROOT/'cb-output/image-trials';MEDIA=ROOT/'engine/media';PREFIX='Ep3_S1_two_shot_trial_v01'
...
def prepare():
 ...
 for n,ids in [(1,range(5)),(2,range(5,12))]:
  out=Image.new('RGB',(640,360*len(ids)),'white')
  for j,i in enumerate(ids):
   x,y=i%3,i//3
   box=(int(x*w/3),int(y*h/4),int((x+1)*w/3),int((y+1)*h/4))
   out.paste(board.crop(box).resize((640,360)),(0,j*360))
...
def video(n):
 ...
 task['storyboard_reading_order']='top to bottom, one column';task['references']=copy.deepcopy(prior['task']['references'][:7]);task['references'][6]['defines']=f'ordered coverage views for THIS clip only, top to bottom; camera and action guide, not initial state'
 ...
 if n==1:
  ...
  task['camera']='Follow @Image 7 coverage top to bottom for launch, impact, laugh and response. End with the explicit wide handoff described below. Use full-screen moving CGI, not a board. Same launch axis: Zenny remains ahead of the berry until impact; cuts change camera, never geography.'
 else:
