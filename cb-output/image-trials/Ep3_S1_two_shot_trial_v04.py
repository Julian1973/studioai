...
BASE=ROOT/'cb-output/image-trials';MEDIA=ROOT/'engine/media';PREFIX='Ep3_S1_two_shot_trial_v04'
...
def prepare():
 ...
 for n,ids in [(1,[0,2,3,4]),(2,range(5,12))]:
...
def video(n):
 ...
 if n==1:
  stages[0]=('0–3s. Begin at EXACT @Image 1. Keen releases the berry once from the near-bank catapult. Immediately track forward WITH the berry toward Zenny on the far bank. Once the travelling view begins, Keen and the catapult are BEHIND the camera and OUT OF FRAME for the entire flight and impact. They remain at their original launch position; do not move, duplicate or carry them alongside the berry. Keep Zenny ahead at her fixed far-bank leaf, becoming larger only as the camera approaches. First contact with her forehead becomes one splat; no overshoot or replay.', 'Zenny on her original far leaf with purple juice. Offscreen behind the camera: Keen beside the near-bank EMPTY machine.')
  ...
  task['camera']='@Image 6 defines four ordered views: setup, splat, Fuzzby laughing, Zenny response. Between setup and splat, the camera travels forward alongside the berry; Keen and the catapult remain behind the camera, outside the flight framing. Never reuse a wide view keeping them on the left during flight. Begin at @Image 1, then end in Zenny’s settled close-up. World positions remain fixed; only the camera travels.'
