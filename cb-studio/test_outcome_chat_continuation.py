...
@pytest.mark.parametrize('scenario', ['approved', 'failed', 'changed-shot', 'changed-scene', 'request-ready', 'chained'])
...
(async()=>{
  ...
  await directorFollowJob(scope,'key',{jobId:'test',decisionKind:scenario==='request-ready'?null:'keyframe'});
  ...
  assert.deepEqual(events,scenario==='chained'?[['voice',0],['chat','voice'],['chat','voice']]:scenario==='approved'?[['voice',0]]:scenario==='request-ready'?['review-request']:[]);
