...
The loop, per the spec's own gate order (voice BEFORE images — timing drives pictures):
  ...
  Gate 8  approve/reject    Julian selects ONE candidate (others archived) — approval
                            ...
                            batch rejection follows THE FAILURE DECISION LADDER; repeated
                            rejection remains advisory and never blocks the shot
...
def _submit_seedance_provider(prompt, image_inputs, **kwargs):
    ...
    output = pathlib.Path(kwargs.get("out") or MEDIA)
    parent = output.parent if output.is_absolute() else MEDIA
    ...
    if 'segmentIndex' in evidence:
        from studio_prompt_quality import require as require_prompt_quality
        require_prompt_quality(prompt, evidence.get('promptQuality'))
    ...
    groups = {'image': image_inputs, 'audio': kwargs.get('audio_urls') or [],
              'video': kwargs.get('video_urls') or []}
    with pinned_media(groups, evidence.get('mediaBindings')) as pinned, capture(ROOT / 'cb-output/state/provider-requests', metadata,
        ...
        kwargs['audio_urls'] = pinned['audio'] if groups['audio'] else None
        if groups['video']:
            kwargs['video_urls'] = pinned['video']
        return cb_gen.generate_video_seedance_ref(prompt, pinned['image'], **kwargs)
...
def _require_current_see_canon(pkg, shot_id=None):
    from studio_see_package import current_tail_issue
    for shot in pkg.get('shots', []):
        if shot_id is None or shot.get('shotId') == shot_id:
...
def _episode_character_state(character, shot, scene, episode):
    ...
    """
    canonical = _resolve_char(character, _characters_cfg())
    if canonical == "Bo" and str(episode) == "Ep2":
        required = {
            str(item or "").strip().casefold()
            for item in ((shot or {}).get("requiredPropReferences") or [])
        }
        try:
            scene_number = int(str(scene))
        except (TypeError, ValueError):
            scene_number = 0
        if ("bo_satchel" in required or "satchel" in required or
                scene_number >= 4):
            return "with-satchel"
        return None
    if _norm(canonical) != _norm("Keen") or str(episode) != "Ep1":
        return None
...
def generate_scenelook_plate(scene, episode="Ep1", reference_path=None, log=print, *, reviewed_request=None):
    ...
    """
    if _pkg_path(scene, episode).exists():
        _require_current_see_canon(load_pkg(scene, episode)[0])
    st = scenelook_status(scene, episode)
    if st["candidate"]:
    ...
    from studio_scene_plate import native_request, assert_current, verify_transport
    compiled = native_request(ROOT, scene, episode, reference_path=reference_path)
    if not reviewed_request:
        raise Refused('Review the compiled scene plate through SEE: View prompt & references before Generate.')
    ...
    out.with_suffix('.scene-plate-request.json').write_text(json.dumps(receipt, indent=2))
    from studio_seedream_size import crystal_bears_aspect
    cb_gen.generate_image(prompt, refs=refs, aspect=compiled["settings"]["aspect"], out=str(out), image_size="2K", production_route="cb_render")
    receipt.update(status="provider_returned", outputHash=_sha256_file(out))
    out.with_suffix(".scene-plate-request.json").write_text(json.dumps(receipt, indent=2))
    assert_current(compiled, native_request(ROOT, scene, episode, reference_path=reference_path))
    ...
    rec["candidate"] = {"path": str(out), "hash": _sha256_file(out),
                        ...
                        "generatedAt": _now(), "approvalStatus":"PENDING_HUMAN_REVIEW",
                        "scenePlateRequestHash":compiled["requestHash"]}
...
def _opening_composition_contract(pkg, shot, scene, episode, characters_cfg):
    ...
    # DIRECT owns new SEE composition, just as it owns the emitted prompt. Retain
    # the specialist record only for historical shots without a DIRECT opening.
    card = shot.get("directorCard") or {}
    has_direct_opening = any((shot.get("openingPose"),
                             shot.get("openingFrameLayoutApproved"),
                             shot.get("openingFrameLayout"),
                             card.get("openingFrameLayout"),
                             (card.get("views") or [{}])[0].get("staging")))
    direction = (_direct_keyframe_direction(shot) if has_direct_opening else
                 _inspection_department_output(
...
def _keyframe_request_preflight(pkg, shot, scene, episode, *, compare=False):
    """Check the exact provider-bound SEE request without changing state or spending."""
    _require_show_adapter()
    _require_current_see_canon(pkg, shot["shotId"])
    _require_valid(pkg)
    _require_current_lineage(pkg, scene, episode)
    _require_confirmed_billing("byteplus")
    if compare:
    ...
    playable = cb_engine_rules.playable_stage_report(
        shot, _direct_keyframe_direction(shot))
    if not playable["ready"]:
        raise Refused("REFUSED — opening frame is not a playable stage: "
    ...
    contract = _keyframe_prompt_contract(pkg, shot, prompt)
    return {"characters": characters_cfg, "references": refs, "prompt": prompt,
            "signature": signature, "contract": contract}


def keyframe_build_status(scene, shot_id, episode="Ep1", *, compare=False):
    """Read-only plan; one Seedream candidate unless comparison is explicitly requested."""
    pkg, _ = load_pkg(scene, episode)
    shot = _shot(pkg, shot_id)
    ledger = _ledger(pkg, shot_id)
    ...
    if ledger.get("keyframeCandidate") or ledger.get("keyframeCandidates"):
        ...
        try:
            ...
            current = bool(
                pending.get("inputSignature") == current_signature and
                pending.get("contentHash") and
                pending["contentHash"] == _sha256_file(pending.get("path")))
        except (Refused, OSError, ValueError):
            current = False
        if current:
            return {
                "state": "ready-for-review", "buildable": False,
                "reason": "The current keyframe candidate is waiting for your decision.",
                "mediaCallsRequired": 0, "maxMediaCalls": 0,
                "estimatedMaxUsd": 0.0, "poseCallsRequired": 0,
                "humanDecision": "Review the returned candidate, then Approve or Reject.",
                "noAutomaticRetries": True,
            }
        stale_candidate = True

    try:
        _keyframe_request_preflight(pkg, shot, scene, episode, compare=compare)
    except (Refused, OSError, ValueError) as exc:
        issue = str(exc).removeprefix("REFUSED — ")
        return {
            "state": "needs-preparation", "buildable": False,
            "reason": f"Before Build, resolve this opening: {issue}",
            "issue": str(exc), "mediaCallsRequired": 0, "maxMediaCalls": 0,
            "estimatedMaxUsd": 0.0, "poseCallsRequired": 0,
            "humanDecision": "Review the opening with Director, or choose Upload or Library.",
            "noAutomaticRetries": True,
        }
    if stale_candidate:
        return {
            ...
            "reason": (
                "The SEE candidate is missing, changed, or was made against an older "
                "prompt or reference contract. It cannot be approved; Build will preserve it as "
                "superseded evidence and build the current Seedream candidate."),
            "mediaCallsRequired": 2 if compare else 1, "maxMediaCalls": 2 if compare else 1,
            "estimatedMaxUsd": None, "poseCallsRequired": 0,
            "humanDecision": "Build the current keyframe, then review it.",
            "noAutomaticRetries": True,
            "staleCandidate": True,
        }

    max_calls = 2 if compare else 1
    try:
        import cb_costs
        ...
        seedream_cost = None
        nb2_cost = None
    estimated = (round(seedream_cost + (nb2_cost if compare else 0), 4)
                 if seedream_cost is not None and nb2_cost is not None else None)
    return {
        "state": "buildable",
        "buildable": True,
        "reason": (
            "The Studio will build Seedream 5 Pro plus Nano Banana 2 for the requested comparison." if compare else
            "The Studio will build one Seedream 5 Pro keyframe from the current prompt and references."),
        "workflow": "dual-provider-see-ab" if compare else "single-seedream-see",
        "posePlan": [],
        "poseCallsRequired": 0,
        "finishingCallsRequired": max_calls,
        "mediaCallsRequired": max_calls,
        "maxMediaCalls": max_calls,
        "estimatedUnitUsd": {"A": seedream_cost, **({"B": nb2_cost} if compare else {})},
        ...
        "provider": "byteplus+google" if compare else "byteplus",
        "providerModelId": {
            "A": cb_gen.SEEDREAM_MODEL_ID,
            **({"B": cb_gen.IMAGE_MODEL} if compare else {}),
        },
        "validatorModel": cb_departments.cb_llm.VALIDATOR_MODEL,
        "stopPolicy": (
...
def build_keyframe(scene, shot_id, episode="Ep1", log=print, *, compare=False):
    ...
    # Keep an old candidate intact if current request preparation would refuse.
    _keyframe_request_preflight(pkg, shot, scene, episode, compare=compare)
    ledger = _ledger(pkg, shot_id)
    pending = ledger.get("keyframeCandidate") or (
        (ledger.get("keyframeCandidates") or [None])[0])
    if pending:
        # A candidate from an older plate/cast/reference graph must not trap the shot
        # between the stale-input warning and the pending-review gate. Preserve it as
        # history, then let Build create a candidate for the current inputs. Current
        # candidates still require the explicit Accept or Iterate decision.
        try:
            current_signature = _keyframe_input_signature(pkg, shot, scene, episode)
            candidate_current = pending.get("inputSignature") == current_signature
        except (Refused, OSError, ValueError):
            ...
            candidate_current = False
        candidate_integrity = bool(pending.get("contentHash") and
                                   pending["contentHash"] == _sha256_file(pending.get("path")))
        if not candidate_current or not candidate_integrity:
            ...
            ledger["keyframeCandidates"] = []
            ledger["selectedKeyframeCandidateId"] = None
            _save(pkg, _pkg_path(scene, episode))
        else:
            raise Refused(
                f"REFUSED — {shot_id} already has a finished keyframe waiting for Accept or Iterate")

    result = keyframe_shot(scene, shot_id, episode, log=log, compare=compare)
    log(f"KEYFRAME BUILD COMPLETE — {shot_id}: "
        + ("SEE A/B is ready for comparison and explicit selection" if compare else
           "the SEE candidate is ready for review")
        + "; no downstream animation was fired")
    return result
...
def _is_relay_or_split_shot(shot):
    # A cut owns its opening even when descended from a split production unit.
    if (shot.get("shotTransition") or {}).get("type") == "cut":
        return False
...
def _continuation_reference_slots(shot, include_audio=False):
    ...
    for character in animation_required_cast(shot):
...
def _with_required_prop_slots(slots, shot, scene, episode):
    """Append explicit prop authority to an image slot map exactly once."""
    slots = dict(slots or {})
    existing_roles = {
    ...
    for role in _required_prop_reference_roles(shot, scene, episode):
...
def _effective_reference_slots(pkg, shot, slots_key, scene, episode):
    ...
    # Append them to every provider bundle after resolving the shot's ordinary roles so
...
def _animation_reference_contract(attachment_plan, shot, audio_path=None):
    ...
    for item in attachment_plan or []:
        ...
        if not tag or not role:
            continue
        if role == "previous shot state reference" or (role == "previous shot final frame" and
                (shot.get("shotTransition") or {}).get("type") == "cut"):
            typed_role = "continuity_state"
            controls = "preceding accepted world, prop and action state only; recompose the new view without copying camera framing"
            scope = "continuity"
        elif role in ("opening keyframe", "previous shot final frame"):
            ...
            scope = "continuity"
        elif role.startswith("storyboard multi-grid"):
            typed_role = "style"
            controls = "chronological multi-grid plot reference: composition, camera progression, causal states and landing; render a full-frame moving scene, never grid lines or labels; DIRECT action remains authoritative"
            scope = "continuity"
        elif role.startswith("storyboard "):
            typed_role = "style"
            controls = f"soft visual guidance at {item.get('atSec')}s: {item.get('state', 'authored checkpoint')}; never exact endpoint interpolation or next-beat action"
            scope = "continuity"
            ...
            scope = "episode"
        elif role.startswith("vision:"):
            typed_role = "prop"
            controls = ("the exact pictured scene as vision content on the water surface only; "
                        "preserve its subjects and arrangement inside the reflection. "
                        "Do not transfer people, furniture, geography or weather to the surrounding pool. "
                        "The current shot owns appearance timing and ripple distortion.")
            scope = "continuity"
...
_NON_IDENTITY_IMAGE_ROLES = {
    "scene plate", "opening keyframe", "previous shot final frame", "previous shot state reference",
...
def _is_non_identity_image_role(role):
    ...
    return (role in _NON_IDENTITY_IMAGE_ROLES or role.startswith("prop:")
            or role.startswith("location:") or role.startswith("vision:"))
...
def _expanded_reference_blueprint(shot, slots_key, characters_cfg, scene=None,
    ...
    for slot, role in raw_slots.items():
        ...
        if role in ("previous shot final frame", "previous shot state reference"):
            if usage == "keyframe":
                ...
                if (transition.get("type") == "cut" and
                        transition.get("stateSourceShotId")):
                    # A hard cut owns a new opening composition, but the accepted prior
                    # ending still anchors world/action continuity as state-only evidence.
                    role = "previous shot state reference"
                else:
                    # A relay uses the predecessor's exact approved ending as its
                    # opening authority. Keep it in the sealed reference plan.
                    role = "previous shot final frame"
            else:
                role = "opening keyframe"
        slots[slot] = role
    transition = shot.get("shotTransition") or {}
    if (usage == "keyframe" and transition.get("type") == "cut" and
            transition.get("stateSourceShotId") and
            not any(role == "previous shot state reference" for role in slots.values())):
        next_slot = max([int(key[2:]) for key in slots
                         if key.startswith("@图") and key[2:].isdigit()] or [0]) + 1
        slots[f"@图{next_slot}"] = "previous shot state reference"
    ...
    from studio_reference_contract import complete_identity_slots, required_cast, animation_required_cast
    cast = (required_cast(shot) if usage == "keyframe" else
            animation_required_cast(shot))
    slots = complete_identity_slots(slots, cast)
    expanded = []
    source_slots = [key for key in slots if key.startswith("@图")]
    relay_keyframe = usage == "keyframe" and _is_relay_or_split_shot(shot)
    ...
    for source_slot in source_slots:
        ...
        for identity in identities:
            expanded.append({
                ...
                "role": role,
                "usage": usage,
                "stateBinding": _reference_state_binding(shot, slots_key, source_slot, role),
...
def _reference_state_binding(shot, slots_key, source_slot, role):
    """Slot numbers belong to one stage; carry evidence by its logical role."""
    if slots_key == "keyframeReferenceSlots":
        explicit = (shot.get("keyframeReferenceStateBindings") or {}).get(source_slot)
        if explicit is not None:
            return explicit
        source_slot = next((slot for slot, name in (shot.get("referenceSlots") or {}).items()
                            if name == role), None)
    return (shot.get("referenceStateBindings") or {}).get(source_slot, {})
...
def _slot_path_for_role(role, anchor_path, scene, episode, characters_cfg, shot=None,
                        usage="keyframe"):
    if (role == "previous shot final frame" and
            ((shot or {}).get("shotTransition") or {}).get("type") == "cut"):
        # A cut's anchor_path is its NEW opening, never the preceding ending.
        return _slot_path_for_role("previous shot state reference", None, scene,
                                   episode, characters_cfg, shot, usage)
    if role == "previous shot state reference":
        source_id = ((shot or {}).get("shotTransition") or {}).get("stateSourceShotId")
        source_pkg, _ = load_pkg(scene, episode)
        source = _ledger(source_pkg, source_id) if source_id else {}
        relay = source.get("editorialRelayReview") or {}
        frame = relay.get("relayFramePath") or source.get("harvestFrame")
        relay_hash = relay.get("relayFrameSha256") if relay.get("relayFramePath") else None
        if relay_hash and (not frame or not os.path.isfile(frame) or _sha256_file(frame) != relay_hash):
            raise Refused("REFUSED — approved editorial relay frame changed; recover it before preparing the cut")
        target_ledger = next((x for x in source_pkg['continuityLedger']
                              if x.get('shotId') == (shot or {}).get('shotId')), {})
        candidate_source = target_ledger.get('candidateStateSource')
        if (source.get("status") != "approved" or candidate_source) and shot:
        ...
        expected = (None if relay_hash or candidate_source else
                    ((source.get("approval") or {}).get("harvestHash")
        ...
        if not candidate or not _reference_path_is_approved(candidate):
    ...
    if role in ("opening keyframe", "previous shot final frame"):
        ...
        path = matches[0]["path"]
    elif role == "Bo vision plate" or str(role).startswith("vision:"):
        candidates = cb_asset_registry.resolve_assets(
            episode, scene, shot_id=(shot or {}).get("shotId"),
            kinds={"reference_image"})
        matches = [item for item in candidates if str(item.get("role") or "") == role
...
def shot_reference_manifest(scene, shot_id, episode="Ep1"):
    ...
    def image_entries(slots_key, anchor_path=None):
        entries = []
        reference_shot = ({**shot, "referenceSlots": animation_slots}
                          if slots_key == "referenceSlots" else shot)
        blueprint = _expanded_reference_blueprint(
            reference_shot, slots_key, characters_cfg, scene=scene, episode=episode)
        for attachment in blueprint:
            ...
            try:
                if identity:
                    ...
                    path = _slot_path_for_role(
                        role, anchor_path, scene, episode, characters_cfg, shot=reference_shot,
    ...
    def unavailable_from_slots(slots_key, message):
        raw = (animation_slots if slots_key == "referenceSlots"
               else dict(shot.get(slots_key) or {}))
...
def _with_effective_dialogue_timing(shot, led):
    ...
    if not lines or not cb_audio_authority.spoken_dialogue_lines(shot):
        return shot
    timing_by_occurrence = O.Index(line for line in lines if line.get('dialogueOccurrenceId'))
...
def _performance_budget_report(shot, led):
    ...
    measured_voice_timing = False
    placement_path = led.get("voPlacementPath")
    placement_error = False
    if placement_path:
        # Raw ElevenLabs timestamps are relative to the source take. Only the
        # assembled Audio1 placement describes speech on the shot timeline.
        try:
            placement = json.loads(pathlib.Path(placement_path).read_text())
            if (placement.get("outputSha256") != _sha256_file(led["voPath"]) or
                    abs(float(placement["durationSec"]) - duration) > 0.05):
                raise ValueError("audio placement does not match this shot")
            measured = []
            for segment in placement.get("placements") or []:
                start = segment.get("targetStartSec")
                end = segment.get("targetEndSec")
                if start is not None and end is not None:
                    measured.append((start, end))
            if not measured:
                raise ValueError("audio placement has no timed dialogue")
            lines = [{"startSec": start, "endSec": end} for start, end in measured]
            measured_voice_timing = True
    ...
    reasons = []
    if placement_error:
        reasons.append("approved audio placement is missing or does not match Audio1")
    decision = str(budget.get("decision") or "")
    if decision != "single-unit" and not decision.startswith("single-unit-"):
    ...
    reserve_adjusted = False
    effective_reserve = reserve
    if (measured_voice_timing and available + 0.05 < required_unvoiced and
            available + 0.05 >= landing):
        # Physical acting can continue under approved dialogue. Protect the
        # landing hold, and use the measured remaining air as the silent reserve
        # instead of stopping a valid single-unit shot for an editorial target.
        effective_reserve = max(0.0, available - landing)
        reserve_adjusted = True
    if available + 0.05 < required_unvoiced and not reserve_adjusted:
        reasons.append(
            f"voice leaves {available:.2f}s unoccupied but acting and landing require "
            f"{required_unvoiced:.2f}s")
    if measured_voice_timing and merged and duration - merged[-1][1] + 0.05 < landing:
        reasons.append(
            f"final voice ends at {merged[-1][1]:.2f}s, leaving "
            f"{duration - merged[-1][1]:.2f}s for the {landing:.2f}s landing hold")
    return {
        "applicable": True,
        "ready": not reasons,
        ...
        "effectiveSilentActingReserveSec": round(effective_reserve, 3),
        "measuredVoiceTiming": measured_voice_timing,
        "reserveAdjustedForDirectedOverlap": reserve_adjusted,
        "emotionalTurnCount": budget.get("emotionalTurnCount"),
        "propStateChangeCount": budget.get("propStateChangeCount"),
        "reasons": reasons,
        "recommendedAction": ("split-at-strongest-story-boundary" if reasons else
                              "proceed-with-directed-overlap" if reserve_adjusted else
                              "proceed-to-animatic"),
    }
...
def _rebase_shot_dialogue_sources(shot, events, script_text, corrected_line_index,
                                  corrected_text):
    """Bind every line in one shot to the current script revision.

    Script occurrence IDs are revision-scoped. A scoped edit must therefore rebind the
    whole shot, not just the edited line, and attach verified spoken spans before HEAR
    can fire. Ambiguous or out-of-order matches fail closed.
    """
    lines = list(shot.get("dialogueLines") or [])
    if not 0 <= corrected_line_index < len(lines):
        raise Refused("REFUSED — the corrected dialogue line is no longer in this shot")
    used, rebound, old_texts = set(), [], [
        str(line.get("exactText") or "").strip() for line in lines]
    previous_event_index = -1
    for index, line in enumerate(lines):
        exact_text = (str(corrected_text or "").strip()
                      if index == corrected_line_index else old_texts[index])
        speaker = str(line.get("speaker") or "").strip()
        matches = [event for event in events
                   if event.get("i") not in used and
                   int(event.get("i", -1)) > previous_event_index and
                   _resolve_char(event.get("speaker"), _characters_cfg()) ==
                   _resolve_char(speaker, _characters_cfg()) and
                   str(event.get("text") or "").strip() == exact_text]
        if len(matches) != 1:
            raise Refused(
                f"REFUSED — {shot.get('shotId')} line {index + 1} does not map to "
                "one ordered current-script dialogue event")
        event = matches[0]
        used.add(event["i"])
        previous_event_index = int(event["i"])
        previous_id = O.identity(line.get("dialogueOccurrenceId"))
        line.update({
            "dialogueOccurrenceId": event["dialogueOccurrenceId"],
            "sourceEventId": event["sourceEventId"],
            "exactText": exact_text,
            "sourceSegmentation": _structural_spoken_source(event, script_text),
        })
        rebound.append((previous_id, event))

    for brief in shot.get("voiceDirectorBrief") or []:
        event = next((event for previous_id, event in rebound
                      if previous_id and previous_id ==
                      O.identity(brief.get("dialogueOccurrenceId"))), None)
        if not event:
            continue
        brief.update({
            "dialogueOccurrenceId": event["dialogueOccurrenceId"],
            "sourceEventId": event["sourceEventId"],
            "exactDialogue": event["text"],
        })
        if corrected_line_index < len(old_texts):
            old_text = old_texts[corrected_line_index]
            if old_text and old_text in str(brief.get("elevenLabsV3Direction") or ""):
                brief["elevenLabsV3Direction"] = brief["elevenLabsV3Direction"].replace(
                    old_text, corrected_text)
    return [event for _, event in rebound], old_texts
...
def apply_scoped_dialogue_correction(scene, shot_id, old_occurrence_id, old_exact_text,
    ...
    matches = [line for line in (shot.get("dialogueLines") or [])
               if O.same(line.get("dialogueOccurrenceId"), old_occurrence_id, shot.get("dialogueLines") or []) and
    ...
    parsed = cb_intake.parse_script(
        script_text, log=lambda *args, **kwargs: None)
    cb_intake._annotate_source_events(parsed["events"], script_version_id)
    scene_events = [event for event in parsed["events"]
                    if str(event.get("scene")) == str(scene) and
                    event.get("type") == "dialogue"]
    corrected_events = [event for event in scene_events
                        if _resolve_char(event.get("speaker"), _characters_cfg()) ==
                        _resolve_char(matches[0].get("speaker"), _characters_cfg()) and
                        str(event.get("text") or "").strip() == new_exact_text]
    if len(corrected_events) != 1:
        raise Refused(
            f"REFUSED — the corrected words do not resolve to one current script event in scene {scene}")
    old_line = dict(matches[0])
    old_event_id = old_line.get("sourceEventId")
    rebound_events, old_texts = _rebase_shot_dialogue_sources(
        shot, scene_events, script_text, line_index, new_exact_text)
    event = rebound_events[line_index]
    line = (shot.get("dialogueLines") or [])[line_index]
    ...
    if voice_source:
        ...
        if len(carried_lines) == len(shot.get("dialogueLines") or []):
            for index, carried_line in enumerate(carried_lines):
                current_line = (shot.get("dialogueLines") or [])[index]
                current_event = rebound_events[index]
                performed = str(carried_line.get("performedText") or
                                old_texts[index])
                if index == line_index:
                    leading_tags = " ".join(re.findall(r"\[[^\]]+\]", performed))
                    performed = (leading_tags + " " + new_exact_text).strip()
                    for recipe in carried_line.get("takeRecipes") or []:
                        recipe["performedText"] = performed
                carried_line.update({
                    "dialogueOccurrenceId": current_event["dialogueOccurrenceId"],
                    "sourceEventId": current_event["sourceEventId"],
                    "exactDialogue": current_line["exactText"],
                    "performedText": performed,
                })
                if index + 1 < len(carried_lines):
                    carried_lines[index + 1]["previousText"] = performed
...
def _structural_spoken_source(event, script):
    """Bind a parsed dialogue event to its unique source span, without guessing action."""
    from studio_source_segmentation import project
    phrase = str(event["text"]).strip()
    pattern = re.compile(r"\s+".join(re.escape(part) for part in phrase.split()))
    matches = list(pattern.finditer(script))
    if len(matches) != 1:
        raise Refused(
            "REFUSED — corrected dialogue needs one unambiguous source-script span")
    match = matches[0]
    return project({"dialogueOccurrenceId": event["dialogueOccurrenceId"],
                    "speaker": event["speaker"], "exactText": phrase}, script,
                   boundary={"scriptRevision": "sha256:" + hashlib.sha256(
                       script.encode("utf-8")).hexdigest(),
                             "occurrenceId": event["dialogueOccurrenceId"],
                             "speaker": event["speaker"],
                             "authority": "structural_line_types",
                             "evidenceId": event["sourceEventId"],
                             "spans": {"spokenText": [match.start(), match.end()]}})
...
def apply_scoped_voice_contract_correction(scene, shot_id, corrected_lines,
    ...
    used = set()
    normalized = []
    previous_lines = list(shot.get("dialogueLines") or [])
    previous_briefs = {item.get("dialogueOccurrenceId"): item
    ...
    for index, submitted in enumerate(corrected_lines, start=1):
        ...
        performance_text = str(submitted.get("performanceText") or exact_text).strip()
        if not cb_voice_director.same_spoken_words(
                cb_gen._eleven_voice_text(exact_text),
                cb_gen._eleven_voice_text(performance_text)):
            raise Refused(
                f"REFUSED — corrected voice line {index} changes spoken words. "
                "Update the script dialogue first; keep performance text to acting cues.")
    ...
    shot["dialogueBinding"] = (
        "Perform only the ordered approved dialogue occurrences. Screenplay action, sound "
        "labels and BEAT are silent. Only the active speaker moves their mouth; every other "
        "character reacts silently.")
    briefs = []
    for index, line in enumerate(normalized):
        old = previous_lines[index] if index < len(previous_lines) else {}
        carried = previous_briefs.get(old.get("dialogueOccurrenceId")) or {}
        if (old.get("speaker") != line["speaker"] or
                not str(old.get("exactText") or "").startswith(line["exactText"])):
            carried = {}
        reviewed_brief = corrected_lines[index].get("voiceDirectorBrief")
        if reviewed_brief is not None:
            if not isinstance(reviewed_brief, dict):
                raise Refused(f"REFUSED — corrected voice brief {index + 1} must be a record")
            carried = reviewed_brief
        briefs.append({**carried,
    ...
    shot["voiceDirectorBrief"] = briefs
    shot["audioBrief"] = "\n".join([
...
def _department_candidate(stage, output, context):
    ...
    candidate = {"department": dep, "worker": worker,
            ...
            "output": output,
            "reviewObservations": context.get("reviewObservations") or ""}
...
def prepare_department(scene, stage, shot_id=None, episode="Ep1", log=print):
    ...
    from cb_learning_context import brief as review_observations
    temp_dir = None
    if stage == "look":
        ...
        if stage == "review-animation":
            ...
            if context["originatingProduction"]["status"] == "origin-integrity-failed":
                raise Refused("REFUSED — originating production integrity failed; restore "
                              "the exact sealed batch before review. Existing media remains viewable.")
        if stage == "cinematography":
            ...
            # A scoped shot correction may carry an explicitly authored layout. Use it
            # as the local geometry source even when the director cache returns an older
            # prose response; identity/reference work remains provider-bound below.
            authored_override = shot.get("authoredOpeningFrameLayout")
            if authored_override:
                authored_override = dict(authored_override)
                authored_placements = list(authored_override.get("placements") or [])
                opening_names = list(shot.get("openingCharactersInFrame") or
                                     shot.get("charactersInFrame") or [])
                by_character = {
                    str(item.get("character") or "").strip(): item
                    for item in authored_placements
                    if isinstance(item, dict)
                }
                authored_override["placements"] = [
                    by_character[name] for name in opening_names if name in by_character
                ]
                result = cb_departments.CinematographyDirection.model_validate({
                    **result.model_dump(),
                    "charactersInFrame": list(opening_names),
                    "openingFrameLayout": authored_override,
                })
            ...
            context["latestUserCorrection"] = led.get("voiceCorrectionRequest")
            voice_lines = [
                dict(line) for line in cb_audio_authority.spoken_dialogue_lines(shot)
            ]
            working_by_occurrence = O.Index(((led.get("workingVoice") or {}).get("lines") or []))
            for line in voice_lines:
                override = (working_by_occurrence.get(line.get("dialogueOccurrenceId")) or {}).get("text")
                if override:
                    line["performanceOverride"] = override
            from studio_voice_preparation import current as prepared_voice
            # Human overrides remain authoritative; they need their own current review.
            result = (None if (led.get("workingVoice") or {}).get("lines") or led.get("voiceCorrectionRequest") else
                      prepared_voice(shot, voice_lines))
            if result is None:
                result = cb_departments.prepare_voice(context, voice_lines, log=log)
            else:
                log("VOICE — reused direction-time specialist output; no model call")
            ...
            context["currentCinematographyDirection"] = cinematography
            ...
            placement_path = led.get("voPlacementPath")
            if placement_path and pathlib.Path(placement_path).is_file():
                placement_record = json.loads(pathlib.Path(placement_path).read_text())
                context["approvedAudioPlacements"] = placement_record.get("placements") or []
                context["audioPlacementAuthority"] = (
                    "Use targetStartSec/targetEndSec from approvedAudioPlacements for every "
                    "dialogue timeline event, mapped by dialogueOccurrenceId or dialogueIndex. "
                    "These measured intervals supersede estimated speech durations. "
                    "Do not move or alter the audio. Let the final spoken word settle before rising.")
            director_feedback = str(
                (led.get("watchDirectorFeedback") or {}).get("text") or "").strip()
            if director_feedback:
                context["shot"] = {
    ...
    final_direction_review = None
    if stage in ("cinematography", "voice") and hasattr(result, "model_dump"):
        from studio_final_direction import finalize
        correction = led.get("pendingKeyframeCorrection") or (
            (led.get("keyframeRejections") or [])[-1:] or [{}])[0]
        final_context = {**context, "latestUserCorrection": correction if stage == "cinematography" else led.get("voiceCorrectionRequest")}
        if stage == "voice":
            final_context["lockedVoiceLines"] = voice_lines
        result, final_direction_review = finalize(
            stage, result, final_context,
            images=images if stage == "cinematography" else None, log=log)
        if stage == "voice":
            cb_departments.validate_voice_direction(result, voice_lines)
        else:
            _compile_keyframe_integration_prompt(result.model_dump(), shot)
    ...
    if final_direction_review:
        final_direction_review["outputHash"] = hashlib.sha256(json.dumps(
            result_data, sort_keys=True, ensure_ascii=False, default=str).encode()).hexdigest()
        work["candidate"]["finalDirectorReview"] = final_direction_review
    if stage == "review-animation":
...
def recompile_animation_candidate(scene, shot_id, episode="Ep1", log=print):
    ...
    shot = _shot(pkg, shot_id)
    ledger = _ledger(pkg, shot_id)
    # Script/storyboard migrations can change the scoped shot digest while leaving every
    # cinematography input and authored direction unchanged.  In that one hash-only case,
    # carry the prepared direction forward before rebuilding Animation.  Otherwise the
    # automatic WATCH repair deadlocks behind a specialist refresh that has no creative work
    # to redo.  Any substantive cinematography input change still fails closed below.
    cine_checker = globals().get("_department_record_status")
    if callable(cine_checker):
        cine_state = cine_checker(pkg, shot_id, "cinematography", scene, episode)
        cine_record = cine_state.get("record") or {}
        cine_expected = cine_state.get("expectedInputSignature")
        cine_changes = set(_signature_diff(
            cine_record.get("inputSignature"), cine_expected))
        if (not cine_state.get("current") and cine_record.get("output") and
                cine_expected and cine_changes == {"shotContractHash"}):
            cine_record["inputSignature"] = cine_expected
            cine_record["hashOnlyCarryForward"] = {
                "at": _now(),
                "reason": "scoped shot metadata changed; cinematography inputs unchanged",
                "changedFields": sorted(cine_changes),
            }
    ...
    if ledger.get("pendingSpendAuth"):
        ...
        ledger["pendingSpendAuth"] = None
    if (ledger.get("status") != "approved" and
            (ledger.get("batch") or {}).get("status") == "complete" and
            ledger.get("candidatePaths")):
    ...
    elif (not ledger.get("candidatePaths") and
          ledger.get("status") == "direction-recompiled-candidates-stale"):
        # A rejected batch remains in history as complete even after all of its media
        # has been archived. With no live candidate there is nothing left to invalidate;
        # keep the freshly compiled direction fireable instead of creating a dead gate.
        ledger["status"] = "designed"
...
def _keyframe_frame_section(direction, characters_cfg, revision_target=""):
    """Render authored staging, never presenting derived layout coordinates as approved."""
    layout = direction["openingFrameLayout"]
    staging_lines = []
    scale_facts = []
    derived_layout = direction.get("openingFrameLayoutAuthority") == "derived_advisory"
    for item in layout.get("placements") or []:
        ...
        pose = pose or "approved pose"
        pose = pose.replace("in the approved frame-one staging", "approved pose")
        facing = facing.replace("toward the approved story focus", "toward story focus")
        facing = re.sub(r"^(?:facing|faces)\s+", "", facing, flags=re.I)
        if not derived_layout:
            staging_lines.append(f"- {name} {zone}; {pose}; faces {facing}.")
        try:
            canonical = _resolve_char(name, characters_cfg)
            ...
            pass
    # A scale-specific reviewed correction supersedes the generic scale sentence below.
    # Keep the typed positions and poses in SUBJECTS; the exact corrected ratio remains
    # in MUST PRESERVE. The prompt validator uses this same projection.
    if re.search(r"\b(scale|height|size|ratio)\b", str(revision_target or ""), re.I):
        return "\n".join(staging_lines)
    if layout.get("sameDepth") and len(scale_facts) > 1:
        scale_rule = (
            f"Same depth: {'; '.join(scale_facts)}; preserve relative-size truth only.")
    elif len(scale_facts) > 1:
        scale_rule = (
            f"Physical staging scale: {'; '.join(scale_facts)}; apply the authored "
            "camera depth, not equal screen height.")
    else:
        scale_rule = "Preserve canon scale for the authored depth."
    return "\n".join([*staging_lines, f"- {scale_rule}"])


def _keyframe_prompt_correction_text(value):
    """Keep requested correction; omit repeated reviewer acceptance/lock boilerplate."""
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    markers = ("visible result required:", "keep locked:")
    cut = min((index for marker in markers
               if (index := text.casefold().find(marker)) >= 0), default=len(text))
    return text[:cut].strip(" ;")


def _latest_keyframe_revision_target(pkg, shot_id):
    """Return the latest creative correction shared by prompt compilation and QA."""
    ledger = _ledger(pkg, shot_id)
    corrections = [
        *(ledger.get("keyframeRejections") or [])[-1:],
        ledger.get("pendingKeyframeCorrection") or {},
        ledger.get("keyframeRejected") or {},
    ]
    latest = max(
        (item for item in corrections
         if isinstance(item, dict) and item.get("reason")),
        key=lambda item: str(item.get("recordedAt") or item.get("rejectedAt") or
                             item.get("reviewedAt") or ""),
        default={},
    )
    # Automatic dependency invalidation is provenance, not a producer note.
    # Its wording must never become image direction or a required prompt clause.
    if latest.get("category") == "stale-inputs":
        return ""
    return _keyframe_prompt_correction_text(latest.get("reason"))
...
SEEDREAM_KEYFRAME_PROMPT_STANDARD = "studio-seedream-opening-brief@2.3.1"
SEEDREAM_KEYFRAME_PROMPT_SECTIONS = (
    "PURPOSE", "REFERENCE AUTHORITY", "SUBJECTS", "ENVIRONMENT", "OPENING STATE",
    "COMPOSITION", "LIGHTING", "STYLE", "DIRECTOR INTENT", "MUST PRESERVE",
    "DO NOT SHOW YET", "AVOID", "OUTPUT",
)


def _compile_keyframe_integration_prompt(direction, shot, reference_plan=None):
    """Compile the documented Seedream 5 Pro production brief from approved direction."""
    ...
    for _source_slot, attachments in grouped:
        ...
        if role in ("previous shot state reference", "previous shot final frame"):
            ...
            compact_reference_lines.append(
                f"- {slot}: prior accepted frame for latest visible world and prop "
                "state only; this shot's approved opening controls the next beat and "
                "camera, not prior framing.")
        elif role == "scene plate":
            slot = attachments[0]["slot"]
            scene_look_slot = slot
            compact_reference_lines.append(
                f"- {slot}: fixed set, geography and lighting only; the shot opening "
                "controls movable prop state.")
        elif str(role).startswith("location:"):
            slot = attachments[0]["slot"]
            compact_reference_lines.append(
                f"- {slot}: fixed location and landmarks only.")
        elif str(role).startswith("prop:"):
            slot = attachments[0]["slot"]
            prop_name = str(role).split(":", 1)[1].replace("_", " ")
            compact_reference_lines.append(
                f"- {slot}: exact {prop_name} design and scale.")
        elif role == CHARACTER_SCALE_CONTROL_ROLE:
            slot = attachments[0]["slot"]
            compact_reference_lines.append(
                f"- {slot}: canon scale only; no extra subject.")
        else:
            canonical = _resolve_char(role, characters_cfg)
            identity_names.append(canonical)
            if len(attachments) == 1:
                item = attachments[0]
                identity_reference_groups.append(f"{item['slot']} {canonical}")
                collapse_bindings.append((item["slot"], canonical))
            else:
                view_bindings = ", ".join(
                    f"{item['slot']} {item.get('view') or 'identity'}"
                    for item in attachments)
                identity_reference_groups.append(f"{canonical} ({view_bindings})")
                collapse_bindings.append((
                    "/".join(item["slot"] for item in attachments), canonical))

    if identity_reference_groups:
        compact_reference_lines.insert(
            0, "- Canon identity/proportions only: " +
            "; ".join(identity_reference_groups) + ".")

    collapse_line = (emission.multi_angle_collapse_summary(collapse_bindings)
                     if any(len(items) > 1 for _, items in grouped) else "")
    if collapse_line:
        compact_reference_lines.insert(0, f"- {collapse_line}")
    ...
    revision_target = _keyframe_prompt_correction_text(
        direction.get("latestRevisionTarget"))
    scale_protection = _keyframe_same_depth_scale_protection(
        direction, characters_cfg)
    if scale_protection and not re.search(
            r"\b(scale|height|size|ratio)\b", revision_target, re.I):
        protections.insert(0, scale_protection)
    if revision_target:
        protections.append(
            f"Latest reviewed correction: {revision_target} It overrides older prompt "
            "wording for this issue only; the approved shot card remains authoritative.")
    if not protections:
        protections.append("Keep approved staging, direction and designs.")
    object_lifecycle_rules = studio_prompt_aliases.honeycomb_lifecycle_prompt_rules(shot)
    protections.extend(object_lifecycle_rules.get("preserve") or [])
    ...
    frame = _keyframe_frame_section(direction, characters_cfg, revision_target)
    geography = "\n".join(contract["geography"])
    negative_space = "\n".join(contract["negativeSpace"])

    scene_look_authority = (
        f"{scene_look_slot} sets the approved set and light."
        if scene_look_slot else "Use the approved visible location.")

    def _emit():
        cast = contract["cast"]
        cast_count = ", ".join(f"one {name}" for name in cast)
        camera = emission.ensure_complete_sentence(direction["lensAndCameraRelationship"],
                                                    context="keyframe camera direction")
        # Only authored opening state is rendered; never infer it from the later payoff.
        opening = (shot.get("openingState") or direction.get("openingState") or
                   shot.get("openingPose") or "Show the approved opening moment.")
        if not isinstance(opening, str):
            opening = json.dumps(opening, ensure_ascii=False)
        transition = shot.get("shotTransition") or {}
        if transition.get("type") == "cut" and transition.get("stateSourceShotId"):
            source_id = transition["stateSourceShotId"]
            handoff = (
                f"Hard cut from {source_id}: carry world/prop/action/emotion only; "
                "use this shot's opening and camera, never prior framing.")
        else:
            handoff = production_contracts.shot_handoff_instruction(shot, still=True)
        deferred = shot.get("doNotShowYet") or []
        if isinstance(deferred, str):
            deferred = [deferred]
        if not deferred and re.search(r"\b(?:not yet|before|prior to|pre-contact)\b",
                                      str(direction.get("openingState") or ""), re.I):
            deferred = [f"Do not advance beyond the approved start state: "
                        f"{direction['openingState']}"]
        sections = [
            ("PURPOSE", f"Opening keyframe for {shot.get('shotId')}.") ,
            ("REFERENCE AUTHORITY", compact_reference_body),
            ("SUBJECTS", f"Show exactly {cast_count}.\n" + frame),
            ("ENVIRONMENT", (scene_look_authority + "\n" if scene_look_authority else "") + geography),
            ("OPENING STATE", opening + ("\n" + handoff if shot.get("shotTransition") else "") + "\nFrame one only."),
            ("COMPOSITION", camera + "\n" + negative_space),
            ("LIGHTING", str(direction['lightingAndDepth']).strip()),
            ("STYLE", "Match Crystal Bears canon and the approved Scene Look."),
            ("DIRECTOR INTENT", intended_read),
            ("MUST PRESERVE", "\n".join(protections)),
            ("DO NOT SHOW YET", "\n".join(str(x) for x in deferred) or
             "No action after this opening frame."),
            ("AVOID", "No duplicates, identity drift, malformed anatomy, unapproved props, "
             "text or watermarks. "
             + " ".join(object_lifecycle_rules.get("exclude") or [])),
            ("OUTPUT", "16:9 still, 2K PNG; no text."),
        ]
        return "\n\n".join(f"[{name}]\n{body.strip()}" for name, body in sections)

    # Emit the complete documented brief. Prompt length never selects a shorter variant.
    prompt = _emit()
    # The approved typed opening layout and camera above are the still-image brief.
    # Do not append the raw storyboard panel: it contains later performance/movement
    # fields and nested cinematography data that can contradict this exact frame.
    ...
    for name, body in parsed.items():
        if name in {"COMPOSITION AND DECISIVE INSTANT", "CAMERA"}:
...
def _resolve_keyframe_prompt(pkg, shot):
    ...
    plan = _expanded_reference_blueprint(
        shot, "keyframeReferenceSlots", _characters_cfg())
    correction = _latest_keyframe_revision_target(pkg, shot["shotId"])
    work.pop("latestRevisionTarget", None)
    if correction:
        work["latestRevisionTarget"] = correction
...
def _direct_keyframe_direction(shot):
    ...
    card = shot.get("directorCard") or {}
    # SEE precedes the timed WATCH handoff. Read the same approved opening
    # coverage even when the legacy package has not acquired a Director Card.
    first = (card.get("views") or shot.get("storyboardInternalShotPlanApproved") or [{}])[0]
    cinema = first.get("cinematography") or {}
    camera = first.get("framing") or first.get("framingAndCamera")
    camera_fields = (("angle", "Angle"), ("lens", "Lens"),
                     ("focus", "Focus"), ("composition", "Composition"))
    camera_details = []
    camera_source = re.sub(r"\s+", "", str(camera or "")).casefold()
    for key, label in camera_fields:
        value = str(cinema.get(key) or "").strip()
        if not value:
            continue
        normalized_value = re.sub(r"\s+", "", value).casefold()
        if normalized_value and normalized_value in camera_source:
            continue
        if key == "focus" and re.search(
                r"\b(then|before|after|rack|shift|transfer)\b", value, re.I):
            continue
        camera_details.append(f"{label}: {value}")
    # A still depicts the opening instant, not a move, later cut or action montage.
    if camera_details:
        camera = " ".join([
            str(camera or "").strip().rstrip("."), *camera_details]).strip() + "."
    ...
    if not layout and opening_cast and (shot.get("openingPose") or first.get("staging")):
        ...
        layout = {"referenceCharacter": opening_cast[0],
                  "referenceHeightFraction": .3,
                  # Multiple characters do not prove equal camera depth. Only use
                  # a same-depth relation when DIRECT explicitly authored it.
                  "sameDepth": bool(shot.get("openingSameDepth") or first.get("sameDepth")),
                  "placements": [{
            "character": name, "centerX": (index + 1) / (count + 1),
            "centerY": .6, "depthPlane": 0,
            # The shot-level opening description appears once in OPENING STATE.
            # Do not copy the whole scene sentence into every character's pose
            # and facing fields.
            "pose": "in the approved frame-one staging",
            "facing": "toward the approved story focus",
    ...
    style_version, style_text = cb_departments.canonical_style_paragraph()
    # A SEE still is one authored shot opening, not a summary of the whole scene.
    # Scene-level depth strategies can name characters/actions absent from this
    # frame (for example later beats); only shot/view-local blocking belongs here.
    placements = (authored_layout or {}).get("placements") or []
    typed_staging = bool(placements) and all(
        str(item.get("pose") or "").strip() and
        str(item.get("facing") or "").strip() for item in placements)
    geography = [value for value in (
        None if typed_staging else first.get("staging"), first.get("continuity")) if value]
    if not geography:
        geography = [
    ...
    authored_light = str(cinema.get("light") or cine.get("lightingFunction") or "").strip()
    atmosphere = str(cinema.get("atmosphere") or "").strip()
    opening_timing = " ".join(str(value or "") for value in (
        first.get("startState"), first.get("timing"), shot.get("openingState"),
        shot.get("openingPose")))
    pre_contact = re.search(
        r"\b(?:not\s+yet|before|prior\s+to|pre[- ]?contact)\b"
        r"[^.;]{0,80}\b(?:collid\w*|struck|hit|impact|contact)\b",
        opening_timing, re.I)
    action_triggered_light = re.search(
        r"\b(?:as|when|once|after)\b[^.;]{0,80}"
        r"\b(?:struck|hit|impact|contact|collid\w*|bump\w*)\b",
        authored_light, re.I)
    if pre_contact and action_triggered_light:
        # A still must show frame-one light, not preview a lighting change caused by
        # a later impact. Keep the approved card intact; the animation prompt owns the
        # authored change once that contact occurs.
        lighting_and_depth = (
            "Use the approved Scene Plate's current lighting at frame one; defer the "
            "action-triggered change until contact.")
    else:
        lighting_and_depth = " ".join(
            value for value in (authored_light, atmosphere) if value
        ) or "The approved Scene Plate owns lighting and depth."
    negative_space = list(shot.get("negativeSpace") or [])
    if not negative_space:
        negative_space = [
            "Keep every approved subject and story-critical prop clearly visible."]
    return {
        "audienceRead": first.get("audienceNeed") or first.get("purpose") or card.get("audienceFocus") or
                        shot.get("purpose"),
        "lensAndCameraRelationship": camera or
                                     cine.get("providerInstruction") or shot.get("camera"),
        "lightingAndDepth": lighting_and_depth,
        ...
        "openingFrameLayoutAuthority": (
            "authored" if authored_layout else "derived_advisory"),
        "audienceIntent": first.get("audienceNeed") or first.get("purpose") or shot.get("purpose"),
...
def _voice_performance_word_validation(spoken_lines, performance_lines):
    if len(spoken_lines) != len(performance_lines):
        return {"ready": False, "message": (
            "Voice lines no longer match the current script. Refresh HEAR direction before creating audio.")}
    for index, (source, performance) in enumerate(zip(spoken_lines, performance_lines), start=1):
        source_text = source.get("exactText") if source.get("exactText") is not None else source.get("text")
        prompt_text = performance.get("text") or performance.get("performedText") or ""
        source_text = cb_gen._eleven_voice_text(_voice_word_text(source_text))
        prompt_text = cb_gen._eleven_voice_text(prompt_text)
        if not cb_voice_director.same_spoken_words(source_text, prompt_text):
            return {"ready": False, "message": (
                f"Line {index} changes spoken words. Edit Dialogue above and choose Save corrected words; "
                "that versions the script and reopens HEAR/WATCH together. Keep this prompt to acting tags, "
...
def _voice_output_for_review(pkg, shot):
    """Read current reviewed direction or a validated direction-time draft.

    If no current specialist draft exists, keep the producer's exact DIRECT prompt
    usable. HEAR exposes the missing acting cue; the paid call must still match the
    exact prompt shown for producer confirmation.
    """
    from studio_voice_preparation import current as prepared_voice
    try:
        output = _approved_department_output(pkg, shot["shotId"], "voice") or {}
    except Refused as exc:
        prepared = prepared_voice(shot)
        if prepared is None:
            if "Prepare current Voice specialist direction for" not in str(exc):
                raise
            return {}, "direct-current"
        return prepared.model_dump(), "direction-time-draft"
    if output.get("lines"):
        return output, "voice-director-approved"
    prepared = prepared_voice(shot)
    return ((prepared.model_dump(), "direction-time-draft") if prepared is not None
            else ({}, "direct-current"))


def _resolve_voice_lines(pkg, shot):
    """Exact performance precedence: human edit, current specialist, prepared draft."""
    ...
    output, source = _voice_output_for_review(pkg, shot)
    if output.get("lines"):
        return [{"dialogueOccurrenceId": x.get("dialogueOccurrenceId"),
                 ...
                 "dramaticIntention": x.get("dramaticIntention"),
                 "subtext": x.get("subtext"), "cadenceAndBreath": x.get("cadenceAndBreath"),
                 "timingAndBody": x.get("timingAndBody")}
                for x in output["lines"]], source
    return _default_voice_lines(shot), "direct-current"


def _voice_source_validation(shot):
    """Report whether every source line is safe to send as exact spoken audio."""
    from studio_source_segmentation import SegmentationRequired, spoken
    for index, line in enumerate(shot.get("dialogueLines") or [], start=1):
        try:
            spoken(line)
        except SegmentationRequired:
            speaker = str(line.get("speaker") or "Character").strip()
            return {
                "ready": False,
                "message": (
                    f"{speaker} · line {index} needs its spoken words separated from "
                    "silent script action. Edit the spoken-words card; the remaining "
                    "script text will be kept as action."),
            }
    return {"ready": True, "message": None}


def _safe_voice_delivery_note(delivery, spoken_text):
    """Expose only real V3 tags beside the prompt, never copied script prose/action."""
    value = str(delivery or "").strip()
    if not value:
        return None
    tags = cb_voice_director._TAG_RE.findall(value)
    if not tags:
        return None
    plain = " ".join(cb_voice_director._TAG_RE.sub("", value).split()).casefold()
    spoken = " ".join(str(spoken_text or "").split()).casefold()
    if not spoken or spoken not in plain:
        return None
    return " ".join("[" + tag.strip() + "]" for tag in dict.fromkeys(tags) if tag.strip()) or None


def review_voice_source_boundaries(scene, shot_id, boundaries, episode="Ep1",
                                  reviewed_by="Studio source review", log=print):
    """Bind exact spoken/action spans to the current script without changing its text.

    This is a review-only repair for legacy HEAR cards whose dialogue event contains
    screenplay action. Clean dialogue lines are bound to their structural script spans;
    only explicitly reviewed partitions may separate speech from action.
    """
    import cb_intake
    from studio_source_segmentation import project

    if not isinstance(boundaries, dict) or not boundaries:
        raise Refused("REFUSED — source cleanup needs at least one reviewed dialogue boundary")
    reviewer = str(reviewed_by or "").strip()
    if not reviewer:
        raise Refused("REFUSED — source cleanup needs a named reviewer")
    ...
    shot = _shot(pkg, shot_id)
    ledger = _ledger(pkg, shot_id)
    if (ledger.get("voPath") or (ledger.get("voiceApproval") or {}).get("approved") or
            (ledger.get("workingVoice") or {}).get("lines") or shot.get("preparedVoice")):
        raise Refused(
            f"REFUSED — {shot_id} already has voice work; source cleanup will not alter it")

    current = SCRIPT_STORE.current(episode, required=True)
    script = SCRIPT_STORE.content_path(episode).read_text(encoding="utf-8")
    parsed = cb_intake.parse_script(script, log=lambda *args, **kwargs: None)
    cb_intake._annotate_source_events(parsed["events"], current["scriptVersionId"])
    events = [event for event in parsed["events"]
              if str(event.get("scene")) == str(scene) and event.get("type") == "dialogue"]

    lines = list(shot.get("dialogueLines") or [])
    line_ids = [O.identity(line.get("dialogueOccurrenceId")) for line in lines]
    if not lines or len(line_ids) != len(set(line_ids)) or any(not item for item in line_ids):
        raise Refused(f"REFUSED — {shot_id} dialogue identities are missing or ambiguous")
    boundary_by_id = {O.identity(key): value for key, value in boundaries.items()}
    if set(boundary_by_id) - set(line_ids):
        raise Refused(f"REFUSED — source cleanup names a line outside {shot_id}")

    used_events = set()
    previous_index = -1
    projections = []
    for line, occurrence_id in zip(lines, line_ids):
        raw = str(line.get("exactText") or line.get("text") or "").strip()
        speaker = str(line.get("speaker") or line.get("character") or "").strip()
        if not raw or not speaker:
            raise Refused(f"REFUSED — {shot_id} line {len(projections) + 1} is incomplete")
        matches = [event for event in events
                   if event.get("i") not in used_events and
                   int(event.get("i", -1)) > previous_index and
                   _resolve_char(event.get("speaker"), _characters_cfg()) ==
                   _resolve_char(speaker, _characters_cfg()) and
                   str(event.get("text") or "").strip() == raw]
        if len(matches) != 1:
            raise Refused(
                f"REFUSED — {shot_id} line {len(projections) + 1} does not map to one "
                "ordered dialogue event in the current script")
        event = matches[0]
        used_events.add(event["i"])
        previous_index = int(event["i"])
        raw_pattern = re.compile(r"\s+".join(re.escape(part) for part in raw.split()))
        source_matches = list(raw_pattern.finditer(script))
        if len(source_matches) != 1:
            raise Refused(
                f"REFUSED — {shot_id} line {len(projections) + 1} has no unique current-script span")
        source_match = source_matches[0]
        evidence_id = str(event.get("sourceEventId") or "")
        reviewed = boundary_by_id.get(occurrence_id)

        if reviewed is None:
            evidence = {
                "scriptRevision": "sha256:" + hashlib.sha256(script.encode("utf-8")).hexdigest(),
                "occurrenceId": line["dialogueOccurrenceId"], "speaker": speaker,
                "authority": "structural_line_types", "evidenceId": evidence_id,
                "spans": {"spokenText": [source_match.start(), source_match.end()]},
            }
        else:
            if not isinstance(reviewed, dict):
                raise Refused("REFUSED — reviewed source boundary must include spoken text and action")
            spoken_text = str(reviewed.get("spokenText") or "").strip()
            action_before = str(reviewed.get("actionBefore") or "").strip()
            action_after = str(reviewed.get("actionAfter") or "").strip()
            reason = str(reviewed.get("reason") or "").strip()
            if not spoken_text or not reason or not (action_before or action_after):
                raise Refused(
                    "REFUSED — reviewed source boundary needs spoken text, an action span, and a reason")
            spans = {}
            cursor = source_match.start()
            for role, value in (("actionBefore", action_before),
                                ("spokenText", spoken_text),
                                ("actionAfter", action_after)):
                if not value:
                    continue
                part_pattern = re.compile(
                    r"\s*" + r"\s+".join(re.escape(part) for part in value.split()))
                part_match = part_pattern.match(script, cursor, source_match.end())
                if not part_match:
                    raise Refused(
                        f"REFUSED — reviewed {role} does not match the current script in {shot_id}")
                spans[role] = [part_match.start(), part_match.end()]
                cursor = part_match.end()
            if script[cursor:source_match.end()].strip():
                raise Refused(
                    f"REFUSED — reviewed boundary leaves source text unclassified in {shot_id}")
            evidence = {
                "scriptRevision": "sha256:" + hashlib.sha256(script.encode("utf-8")).hexdigest(),
                "occurrenceId": line["dialogueOccurrenceId"], "speaker": speaker,
                "authority": "reviewed_source_boundaries",
                "evidenceId": "boundary-review:" + hashlib.sha256(
                    f"{shot_id}|{occurrence_id}|{reviewer}|{reason}".encode("utf-8")
                ).hexdigest(),
                "reviewedBy": reviewer, "reason": reason, "sourceEventId": evidence_id,
                "spans": spans,
            }
        try:
            projection = project(line, script, boundary=evidence)
        except ValueError as exc:
            raise Refused(f"REFUSED — {shot_id} source boundary is invalid: {exc}") from exc
        existing = line.get("sourceSegmentation")
        if existing is not None and existing != projection:
            raise Refused(
                f"REFUSED — {shot_id} line {len(projections) + 1} already has a different source boundary")
        projections.append(projection)

    changed = any(line.get("sourceSegmentation") != projection
                  for line, projection in zip(lines, projections))
    projection_by_id = {
        occurrence_id: projection
        for occurrence_id, projection in zip(line_ids, projections)
    }
    raw_by_id = {occurrence_id: line.get("exactText", line.get("text", ""))
                 for occurrence_id, line in zip(line_ids, lines)}
    briefs_cleaned = 0
    for brief in shot.get("voiceDirectorBrief") or []:
        occurrence_id = O.identity(brief.get("dialogueOccurrenceId"))
        projection = projection_by_id.get(occurrence_id)
        if projection is None:
            continue
        before = (brief.get("exactDialogue"), brief.get("elevenLabsV3Direction"))
        spoken_text = projection["spokenText"]
        if brief.get("exactDialogue") != spoken_text:
            brief["exactDialogue"] = spoken_text
        direction = str(brief.get("elevenLabsV3Direction") or "").strip()
        tag_prefix = _LEADING_TAG_RE.match(direction)
        tags = tag_prefix.group(1) if tag_prefix else ""
        direction_words = direction[tag_prefix.end():].strip() if tag_prefix else direction
        raw_words = str(raw_by_id.get(occurrence_id) or "").strip()
        if " ".join(direction_words.split()) == " ".join(raw_words.split()):
            brief["elevenLabsV3Direction"] = (tags + spoken_text).strip()
        after = (brief.get("exactDialogue"), brief.get("elevenLabsV3Direction"))
        if before != after:
            briefs_cleaned += 1
            changed = True
    if changed:
        for line, projection in zip(lines, projections):
            line["sourceSegmentation"] = projection
        _save(pkg, path)
    spoken = [projection["spokenText"] for projection in projections]
    actions = [projection.get("actionAfter") for projection in projections]
    log(f"HEAR SOURCE CLEANUP — {shot_id}: {len(boundary_by_id)} reviewed boundary, "
        f"{briefs_cleaned} acting prompt(s) aligned; script preserved; no audio or provider call")
    return {"shotId": shot_id, "existing": not changed,
            "spokenLines": spoken, "actionAfter": actions,
            "briefsCleaned": briefs_cleaned,
            "audioGenerated": False, "providerCalled": False}


def prepare_voice_cues(scene, shot_id, episode="Ep1", log=print):
    """Prepare an editable Luna performance draft; never generate or approve audio."""
    import cb_episode_budget
    import studio_voice_preparation as preparation
    ...
    shot = _shot(pkg, shot_id)
    led = _ledger(pkg, shot_id)
    see_gate(_source_base(), {
        "projectId": pkg.get("projectId") or "crystal-bears",
        "episode": episode, "scene": str(pkg.get("sceneNumber") or scene),
        "unit": shot_id,
    }, approved=True)
    if (led.get("voiceApproval") or {}).get("approved"):
        raise Refused(
            f"REFUSED — {shot_id}'s voice is already approved. Its accepted performance is protected.")
    if (led.get("workingVoice") or {}).get("lines"):
        raise Refused(
            "Your saved ElevenLabs prompt is protected. Edit it in HEAR or restore it before preparing a new draft.")

    spoken_lines = cb_audio_authority.spoken_dialogue_lines(shot)
    if not spoken_lines:
        raise Refused(f"REFUSED — {shot_id} has no spoken lines to prepare for HEAR.")
    validation = _voice_source_validation(shot)
    if not validation["ready"]:
        raise Refused(validation["message"])
    try:
        existing = preparation.current(shot, spoken_lines)
    except (ValueError, RuntimeError) as exc:
        raise Refused(f"REFUSED — saved voice preparation needs review: {exc}") from exc
    if existing is not None:
        log(f"HEAR — {shot_id}: current Luna performance draft reused; no audio generated")
        return {"existing": True, "audioGenerated": False, "draft": shot.get("preparedVoice")}

    context = _shot_context(pkg, shot, led, scene, episode)
    context["latestUserCorrection"] = led.get("voiceCorrectionRequest")
    previous = shot.get("preparedVoice")
    with cb_episode_budget.quote(episode, None, "voice-performance-preparation"):
        prepared = preparation.prepare(
            shot, context=context, previous=previous, log=log)
    # The specialist validates occurrence order, exact script words, canon voice,
    # and allowed V3 tags before this draft can be saved into the visible prompt.
    preparation.current({**shot, "preparedVoice": prepared}, spoken_lines)
    if previous and previous.get("inputHash") != prepared.get("inputHash"):
        shot.setdefault("preparedVoiceHistory", []).append({
            **previous, "status": "superseded", "supersededAt": _now(),
        })
    shot["preparedVoice"] = prepared
    _save(pkg, path)
    log(f"HEAR — {shot_id}: Luna cues ready for producer review; no audio generated")
    return {"existing": False, "audioGenerated": False, "draft": prepared}
...
def voice_performance_status(scene, shot_id, episode="Ep1"):
    ...
    led = _ledger(pkg, shot_id)
    spoken_lines = cb_audio_authority.spoken_dialogue_lines(shot)
    # HEAR must preview the same effective line resolver used by Fire: saved producer
    # edits, current Voice direction, prepared draft, then exact DIRECT fallback.
    directed_output = _voice_output_for_review(pkg, shot)[0]
    direction_records = {O.identity(line['dialogueOccurrenceId']): line
                         for line in directed_output.get('lines') or [] if line.get('dialogueOccurrenceId')}
    for brief in shot.get('voiceDirectorBrief') or []:
        occurrence = brief.get('dialogueOccurrenceId')
        if occurrence:
            direction_records[O.identity(occurrence)] = {
                **direction_records.get(O.identity(occurrence), {}), **brief,
                'cadenceAndBreath': ' · '.join(str(brief.get(k) or '') for k in ('pace', 'rhythm', 'pauses', 'breaths') if brief.get(k)),
                'timingAndBody': brief.get('physicalActionRelationship'),
            }
    direction_by_occurrence = O.Index(direction_records.values())
    approved = []
    ...
    for ln in spoken_lines:
        ...
        approved.append({
            ...
            "scriptExactText": ln.get("scriptExactText", exact_text),
            "delivery": _safe_voice_delivery_note(ln.get("delivery"), exact_text),
    ...
    if not has_take:
        ...
        if not match:
            # Compare the payload that actually reaches ElevenLabs. The take receipt
            # also stores compiler lineage (recipe/hash/settings); current dialogue
            # lines intentionally do not. Comparing those unlike shapes marked a
            # just-generated take stale and blocked its approval.
            chars = cb_voice_director.voice_cards().get("characters", {})
            def provider_projection(lines):
                projected = []
                for line in lines or []:
                    speaker = str(line.get("speaker") or "").strip()
                    character = chars.get(speaker) or chars.get(
                        _resolve_char(speaker, _characters_cfg())) or {}
                    projected.append({
                        "text": line.get("text"),
                        "voiceId": line.get("voiceId") or character.get("voiceId"),
                    })
                return projected
    ...
    try:
        compiler["track"] = cb_voice_director.compile_direct_track(shot, spoken_lines)
        compiler["ready"] = True
    ...
    performance_validation = _voice_performance_word_validation(spoken_lines, current)
    source_validation = _voice_source_validation(shot)
    if source_validation["ready"] and not performance_validation["ready"]:
        source_validation = performance_validation
    working_lines = (working or {}).get("lines") or []
    working_is_current = bool(working_lines) and len(working_lines) == len(current) and all(
        saved.get("dialogueOccurrenceId") == live.get("dialogueOccurrenceId") and
        saved.get("text") == live.get("text")
        for saved, live in zip(working_lines, current))
    return {"approvedLines": approved, "currentLines": current_with_direction, "source": source,
            "sourceValidation": source_validation,
            "isWorking": working_is_current,
...
def select_voice_audition(scene, shot_id, candidate_id, episode="Ep1",
    ...
    directed_line = next((line for line in (direction.get("lines") or [])
                          if O.same(line.get("dialogueOccurrenceId"), occurrence, shot.get("dialogueLines") or [])), None)
...
def save_voice_working(scene, shot_id, lines, episode="Ep1", reviewed_by="Julian", log=print):
    ...
    for i, (ln, dl_ln) in enumerate(zip(lines, dl)):
        ...
        if submitted_id and not O.same(submitted_id, dl_ln.get("dialogueOccurrenceId"), dl):
    ...
    validation = _voice_performance_word_validation(dl, clean)
    if not validation["ready"]:
        raise Refused(validation["message"])
...
def voice_shot(pkg, path, shot_id, episode="Ep1", log=print):
    ...
    if not source_validation["ready"]:
        raise Refused("HEAR_SETUP_REQUIRED: " + source_validation["message"])
    spoken_lines = cb_audio_authority.spoken_dialogue_lines(shot)
    if not spoken_lines:
        return None
    ...
    if len(perf_lines) != len(spoken_lines):
        ...
        raise Refused('Voice performance and approved dialogue occurrences differ. '
                      'Refresh this shot\'s Voice direction; do not substitute undirected speech.')
    validation = _voice_performance_word_validation(spoken_lines, perf_lines)
    if not validation["ready"]:
        raise Refused(validation["message"])
    turns = []
    ...
    if kind == "regeneration" and os.path.exists(out):
        ...
        led["voicePrevious"] = {"path": str(dest.relative_to(HERE)),
                                 ...
                                 "supersededAt": _now()}
    from studio_shot_request import project as production_request
    from studio_request_evidence import capture
    from studio_director_card import stage_decisions
    voice_request = production_request('HEAR',
        scope={'projectId':pkg.get('projectId') or 'crystal-bears','episode':episode,'shotId':shot_id},
        source=pkg.get('sourceScript'), direction=stage_decisions(shot, 'hear'),
        dialogue=spoken_lines, prompt=json.dumps(turns, ensure_ascii=False),
        settings={'model':'eleven_v3', 'turns':turns}).record()
    with capture(ROOT / 'cb-output/state/provider-requests',
                 {'route':'cb_render', 'stage':'hear', 'productionRequest':voice_request},
                 direction=stage_decisions(shot, 'hear')):
        cb_gen.eleven_dialogue(turns, out=str(out), generation_kind=kind,
                              production_route="cb_render")
    led['voiceProductionRequest'] = voice_request
    led["voPath"] = str(out)
...
def approve_voice(scene, shot_id, episode="Ep1", reviewed_by="Julian", log=print,
    ...
    status = voice_performance_status(scene, shot_id, episode)
    if status.get("takeMatchesCurrent") is False and not producer_override:
        raise Refused(
            f"REFUSED — {shot_id}'s audio does not match the current HEAR direction; "
            "create and review the current take first")
...
def reject_voice(scene, shot_id, correction, episode="Ep1", reviewed_by="Julian", log=print):
    ...
    led["voiceCorrectionRequest"] = dict(rejection)
    led["voPath"] = None
    led["voiceApproval"] = None
    led["voGeneratedFrom"] = None
    # A formal human rejection applies to every derivative of that performance.
    # Leaving the failed-placement cache live allowed the next voice run to silently
    # recover rejected paid audio instead of generating the requested new take.
    led["voicePlacementFailure"] = None
...
def _keyframe_prompt_contract(pkg, shot, prompt=None):
    ...
    characters_cfg = _characters_cfg()
    revision_target = _latest_keyframe_revision_target(pkg, shot["shotId"])
    required_sections = [
            (re.sub(r"\s+", " ", str(direction["audienceRead"])).strip(),
             "DIRECTOR INTENT"),
            (_keyframe_frame_section(direction, characters_cfg, revision_target),
             "SUBJECTS"),
    ...
    if revision_target:
        required_sections.append((
            f"Latest reviewed correction: {revision_target}", "MUST PRESERVE"))
    for required_text, section_name in required_sections:
    ...
    if scene_slot and scene_slot not in sections["ENVIRONMENT"]:
        raise Refused(
            "REFUSED — keyframe prompt [ART DIRECTION] is not bound to the approved "
            "Scene Look reference")
    for name in direction_contract["cast"]:
        if f"one {name}" not in sections["SUBJECTS"]:
            raise Refused(
                "REFUSED — keyframe prompt [IDENTITY, COUNT AND STATE] must name each "
                "approved in-frame character exactly once")
    if cb_gen.IMAGE_PROVIDER == "seedream":
...
def screen_keyframe_conformance(pkg, shot, candidate_path, scene, episode="Ep1", log=print):
    ...
    layout = direction.get("openingFrameLayout") or {}
    card = shot.get("directorCard") or {}
    authored_layout = (shot.get("openingFrameLayoutApproved") or
                       shot.get("openingFrameLayout") or card.get("openingFrameLayout"))
    ...
    object_lifecycle_rules = studio_prompt_aliases.honeycomb_lifecycle_prompt_rules(shot)
    forbidden.extend(object_lifecycle_rules.get("forbidden") or [])
    for index, attachment in enumerate(attachment_plan, start=2):
        ...
        # Character exclusions stay scoped to identityContracts. Flattening them
        # here wrongly bans another character's canonical jewellery.
    ...
    context = {
        ...
        "openingFrameLayout": layout,
        "openingFrameLayoutAuthority": "authored" if authored_layout else "derived_advisory",
        "audienceRead": direction.get("audienceRead"),
        "orderedImages": ordered_images,
        "forbidden": list(dict.fromkeys(forbidden)),
        "objectLifecycleRules": object_lifecycle_rules,
        "decisionBoundary": (
...
def keyframe_shot(scene, shot_id, episode="Ep1", log=print, *, compare=False):
    """Generate one SEE candidate, or two when comparison is explicitly requested.

    A is Seedream 5 Pro through BytePlus; optional B is Nano Banana 2 through Google. Each lands at
    ...
    shot = _shot(pkg, shot_id)
    led = _ledger(pkg, shot_id)
    request = _keyframe_request_preflight(
        pkg, shot, scene, episode, compare=compare)
    pending = led.get("keyframeCandidate")
    pending_candidates = list(led.get("keyframeCandidates") or ([pending] if pending else []))
    previous_selection = led.get("selectedKeyframeCandidateId")
    refresh_pending = False
    if pending_candidates:
        current_signature = request["signature"]
        refresh_pending = all(
            item.get("source", "generated") == "generated" and
            (item.get("inputSignature") != current_signature or
             (item.get("conformanceScreening") or {}).get("status") != "pass" or
             not item.get("path") or not os.path.exists(item["path"]) or
             item.get("contentHash") != _sha256_file(item["path"]))
            for item in pending_candidates)
        if not refresh_pending:
            raise Refused(f"REFUSED — {shot_id} already has a current keyframe candidate "
                          "awaiting a decision; review or reject it before generating another")
    characters_cfg = request["characters"]
    ...
    prompt_contract = request["contract"]
    from studio_seedream_size import crystal_bears_aspect, dimensions
    production_aspect = crystal_bears_aspect(ROOT)
    production_size = dimensions(production_aspect, "2K")
    specs = [
    ...
    for candidate_id, label, provider, model_id, generator in specs:
        ...
        try:
            ...
            from studio_shot_request import project as production_request, pinned_media
            frame_bindings = [{'path':str(r), 'sha256':_sha256_file(r),
                               'role':(shot.get('keyframeReferenceSlots') or {}).get(f'@图{i+1}', 'reference')}
                              for i,r in enumerate(refs)]
            frame_request = production_request('SEE',
                scope={'projectId':pkg.get('projectId') or 'crystal-bears','episode':episode,'shotId':shot_id},
                source=pkg.get('sourceScript'), direction=stage_decisions(shot, 'see'),
                refs=frame_bindings, prompt=prompt, settings={'model':model_id, 'aspect':production_aspect, 'size':production_size}).record()
            with capture(ROOT / 'cb-output/state/provider-requests',
                         {'route': 'cb_render', 'stage': 'see', 'episode': episode,
                          ...
                          'productionRequest':frame_request},
                         expected_prompt=prompt, direction=stage_decisions(shot, 'see'),
                         expected_media_counts={'image': len(refs)}):
                with pinned_media({'image':refs}, {'image':frame_bindings}) as pinned:
                    generator(prompt, refs=pinned['image'], aspect=production_aspect, out=str(out), production_route="cb_render")
        except BaseException as exc:
            led["keyframeCandidates"] = candidates or pending_candidates
            led["keyframeCandidate"] = (candidates or pending_candidates or [None])[0]
            led["selectedKeyframeCandidateId"] = (
                None if candidates else previous_selection)
            ...
            _save(pkg, path)
            if candidates:
                log(f"SEE PARTIAL — {shot_id}: {len(candidates)} candidate(s) completed; "
                    f"{candidate_id} / {label} failed after the completed candidate was saved "
                    "(awaiting approval; no duplicate refire needed)")
        ...
        if refresh_pending:
            for old in pending_candidates:
                led.setdefault("keyframeHistory", []).append({
                    **old, "outcome": "superseded-by-refreshed-candidate",
                    "supersededAt": _now(),
                })
            refresh_pending = False
    ...
    led["keyframeABTest"] = {
        ...
        "candidateIds": [item[0] for item in specs],
        "selectionRequired": True,
    }
    _save(pkg, path)
    log(f"SEE — {shot_id}: {len(candidates)} candidate(s) returned for review; "
...
def select_keyframe_source(scene, shot_id, mode, episode="Ep1", upload_path=None,
    ...
    # Commit replacement only after the source has been validated and copied.
    # A failed import leaves the pending candidate and approved work intact.
    if led.get("keyframeCandidate"):
        prior = dict(led["keyframeCandidate"])
        prior.update(outcome="superseded-by-source-selection", supersededAt=_now(),
                     supersededBy=source_note["source"], reviewedBy=reviewed_by)
        led.setdefault("keyframeHistory", []).append(prior)
    from studio_keyframe_selection import retire_comparison
    retire_comparison(led, source=source_note["source"], at=_now(), reviewer=reviewed_by)
...
def _see_readiness_source(pkg, shot, ledger, image_path, scene, episode):
    ...
    opening_slot = next((slot for slot, role in (shot.get("referenceSlots") or {}).items()
                         if "opening" in str(role).lower()), None)
    binding = (shot.get("referenceStateBindings") or {}).get(opening_slot, {})
    candidate = {"path": str(image_path), "sha256": _sha256_file(pathlib.Path(image_path)),
                 "role": "opening keyframe", **bound_metadata(binding, _file_md5(image_path))}
    # SEE may translate legacy coverage for review without modifying the source
    # inputs that generated this image. WATCH prepares its own durable handoff.
    from studio_request_evidence import digest
    prepared = ledger.get("seeDirectorHandoff") or {}
    review_shot = shot
    if (prepared.get("sourceHash") == digest(shot)
            and prepared.get("openingHash") == _sha256_file(image_path)):
        review_shot = prepared["shot"]
        opening_slot = next((slot for slot, role in (review_shot.get("referenceSlots") or {}).items()
                             if "opening" in str(role).lower()), None)
        binding = (review_shot.get("referenceStateBindings") or {}).get(opening_slot, {})
        candidate.update(bound_metadata(binding, _file_md5(image_path)))
    source = snapshot(review_shot, direction, candidate, references)
    source["reviewImageInputs"] = [{"path": str(image_path), "role": "ACTUAL OPENING KEYFRAME TO ASSESS"}] + [
        {"path": r["path"], "role": "SCOPED REFERENCE ONLY: " + str(r.get("role") or r.get("slot") or "reference")}
        for r in references]
    return source


def _approved_see_readiness_report(source):
    candidate = source.get("candidate") or {}
    states = candidate.get("depictedStates") or {}
    errors = (source.get("dynamicStateResolution") or {}).get("errors") or []
    if not candidate.get("stateEvidenceHash") or errors:
        return None
    if not states:
        return None
    from studio_keyframe_director import VERSION as version, digest as see_digest
    return {
        "verdict": "READY",
        "summary": "Approved SEE opening state evidence is current for the opening keyframe.",
        "camera": "Opening keyframe is the approved SEE opening authority; references remain scoped references.",
        "geography": "Approved opening-state binding supplies current geography evidence.",
        "pose": "Approved opening-state binding supplies current character pose evidence.",
        "propsEffects": "Approved opening-state binding supplies current prop/effect evidence; later motion remains authored action.",
        "actionFeasibility": "Current approved SEE evidence supports request preparation without a second visual reviewer overriding the approved opening.",
        "findings": ["approved SEE opening evidence current", "candidate image hash matches recorded state evidence"],
        "correctiveAction": "None.",
        "version": version,
        "inputHash": see_digest(source),
        "scope": "opening-image action readiness",
        "approval": "approved-see-state-evidence",
        "recognition": "file-bound approved SEE state evidence; no new image-recognition call"
    }
...
def review_see_action_readiness(pkg, shot, ledger, image_path, scene, episode):
    ...
    import cb_llm
    from studio_director_handoff import errors as handoff_errors, prepare_native
    if handoff_errors(shot):
        # The returned candidate is available for observation before approval.
        # Complete the source-bound handoff here; never require its own approval
        # as a prerequisite, and never mark the candidate approved to obtain it.
        from copy import deepcopy
        from types import SimpleNamespace
        from studio_request_evidence import digest
        prepared = ledger.get("seeDirectorHandoff") or {}
        if (prepared.get("sourceHash") != digest(shot)
                or prepared.get("openingHash") != _sha256_file(image_path)):
            _, package_path = load_pkg(scene, episode)
            review_pkg = deepcopy(pkg)
            runtime = SimpleNamespace(**{name: getattr(sys.modules[__name__], name)
                for name in ("ROOT", "_shot", "_ledger", "_file_md5", "_sha256_file")})
            runtime._save = lambda *args: None
            runtime.load_pkg = lambda *args: (review_pkg, package_path)
            prepare_native(runtime, scene, shot["shotId"], episode,
                           opening_image=str(image_path), package=(review_pkg, package_path))
            ledger["seeDirectorHandoff"] = {"sourceHash": digest(shot),
                "openingHash": _sha256_file(image_path),
                "shot": deepcopy(_shot(review_pkg, shot["shotId"])),
                "scope": "derived SEE review only; no generation input or approval change"}
    ...
    try:
        ...
        pass
    approved_report = _approved_see_readiness_report(source)
    if approved_report:
        ledger["seeActionReadiness"] = approved_report
        _, package_path = load_pkg(scene, episode)
        _save(pkg, package_path)
        return approved_report
    images = [item["path"] for item in source["reviewImageInputs"]]
    from studio_prompt_director import _review_request
    provenance = []
    report = assess(source, lambda system, data: _review_request(
        system + "\n\nImage input contract: image 1 is the ACTUAL OPENING KEYFRAME TO ASSESS. Later images are scoped references only; never judge a later reference or scene plate as the candidate opening.",
        data, HERE.parent/'cb-output/state/prompt-director', provenance,
        schema=Assessment, label='keyframe_action_readiness', options={'images': images}))
    report['reviewProvenance'] = provenance
...
def approve_keyframe(scene, shot_id, episode="Ep1", reviewed_by="Julian", log=print):
    ...
    # Human image acceptance is distinct from animation action feasibility.
    # Retain any readiness report; WATCH independently requires a current passing
    # report before provider submission. Approve never runs a model or changes
    # the authored shot to manufacture a pass.
...
def reject_keyframe(scene, shot_id, correction, episode="Ep1", reviewed_by="Julian", log=print):
    ...
    # Once a generated candidate has been rejected, its reviewed correction
    # supersedes any older prompt-only note queued before that candidate.
    led.pop("pendingKeyframeCorrection", None)
...
DECISION_LADDER = """THE FAILURE DECISION LADDER (after reviewing a candidate set):
  ...
  6. Repeated failed batches            -> ADVISORY: keep the shot actionable; revise DIRECT
                                          when needed, or rerun the current sealed package.
                                          No automatic approval and no prompt-patching."""
...
def _bank_animation_prompt(pkg, shot_id, led, *, outcome, candidate=None,
    ...
    return cb_prompt_bank.bank_prompt(
        ...
        metadata={
            "projectId": pkg.get("projectId") or "crystal-bears",
            "sourceBindings": (led.get("batch") or {}).get("envelope", {}).get("sourceBindings"),
...
def _reference_records(shot, imgs):
    ...
    return [
        {**bound_metadata(item.get("stateBinding"), _file_md5(path)),
         ...
         "sameCharacterGroup": ((item.get("identity") or {}).get(
             "turnaroundGroupHash")),
         **({"sha256": item["identity"]["sourceSha256"],
             "canonicalIdentity": {"character": item["identity"]["character"],
                 "path": item["identity"].get("source") or item["identity"]["path"], "hash": item["identity"]["sourceSha256"]},
             "traits": {"distinguishingFeatures": item["identity"].get("distinguishingFeatures", []),
                        "mustNotBorrow": item["identity"].get("mustNotBorrow", [])}}
            if (item.get("identity") or {}).get("sourceSha256") else {}),
         "md5": _file_md5(path)}
...
def _write_prefire_block_evidence(shot, reason, *, corrective_action=None,
                                  package=None, source_bindings=None,
                                  report=None):
    payload = {
        'version': 'prefire-block-evidence-1.0.0',
        'shotId': shot.get('shotId'),
        'reason': str(reason),
        'revision': (package or {}).get('revision'),
        'packageHash': (_shots_hash(package) if package else None),
        'sourceBindings': source_bindings or {},
        'correctiveAction': corrective_action or 'Repair the current shot request and rebuild the affected stage before Fire.',
        'providerCalled': False,
        'spendOccurred': False,
        'createdAt': _now(),
        'report': report,
    }
    digest = hashlib.sha256(json.dumps(payload, sort_keys=True, ensure_ascii=False).encode()).hexdigest()[:32]
    destination = ROOT / 'cb-output/state/pre-fire-blocks' / f"{shot.get('shotId', 'unknown')}_{digest}.json"
    from studio_request_evidence import _write
    _write(destination, payload)
    return str(destination)


def _tracked_object_report(shot, prompt, reference_records=None, *, archive=True):
    import studio_tracked_objects
    report = studio_tracked_objects.audit_prompt(prompt, shot, reference_records or [])
    if archive:
        destination = ROOT / 'cb-output/state/tracked-production-objects' / (
            report.get('objectResolutionHash', 'unhashed') + '.json')
        if not destination.exists():
            from studio_request_evidence import _write
            _write(destination, {
                'version': studio_tracked_objects.VERSION,
                'shotId': shot.get('shotId'),
                'promptSha256': hashlib.sha256(str(prompt or '').encode()).hexdigest(),
                'report': report,
            })
        report = {**report, 'evidencePath': str(destination)}
    return report


def _require_tracked_object_report(shot, prompt, reference_records=None):
    report = _tracked_object_report(shot, prompt, reference_records, archive=True)
    if report.get('status') != 'READY':
        reasons = '; '.join(item.get('reason', 'tracked object failure')
                            for item in report.get('errors') or [])
        block_path = _write_prefire_block_evidence(
            shot, 'tracked production objects need repair before Fire: ' + reasons,
            corrective_action='Correct object identity/lifecycle/source bindings, rebuild the affected SEE/WATCH request, then request a new sealed Fire disclosure.',
            source_bindings={'trackedObjectReport': report.get('evidencePath')},
            report=report)
        raise Refused('REFUSED — tracked production objects need repair before Fire: '
                      + reasons + ' (object review: ' + str(report.get('evidencePath'))
                      + '; pre-fire block: ' + block_path + ')')
    return report
...
def _provider_safe_dialogue_prompt(prompt, dialogue_lines):
    ...
    lock = emission.STANDARD_DIALOGUE_AUDIO_AUTHORITY
...
def _animation_execution_plan(pkg, shot, led, imgs, anchor, fast,
    ...
    def provider_audio_path(master, duration):
        ...
        try:
            ...
            raise Refused(str(exc)) from exc
        # WATCH binds the approved HEAR file byte-for-byte. Timing mismatch
        # belongs upstream; no padding, trimming or re-encoding at submission.
        return master
    ...
    # Normalize legacy stored prompts at the final shared emission boundary. This keeps
    # old episodes on the locked Audio1 template without changing dialogue words or timing.
    parent_prompt = _provider_safe_dialogue_prompt(
        parent_prompt, cb_audio_authority.spoken_dialogue_lines(shot))
    provider_prompt = parent_prompt
    if model_id is None:
        try:
        ...
        if not audit.get("productionReady", audit["status"] == "ready"):
            raise Refused(
...
def _binding_hash(pkg, shot, led, imgs, anchor, candidates, fast,
    ...
    payload = {"shotContractHash": hashlib.sha256(json.dumps(
               ...
               "durationSec": shot["durationSec"],
               "trackedProductionObjects": _tracked_object_report(
                   shot, shot["seedancePrompt"], _reference_records(shot, imgs), archive=False),
               "executionPlan": execution_plan}
    return hashlib.sha256(json.dumps(payload, sort_keys=True).encode()).hexdigest()[:32], per


def _validation_beat_package(pkg, episode):
    """Read the exact signed beats that produced this approved scene package."""
    source = pkg.get("sourceBeatPackage")
    if not source:
        return cb_engine._load_pkg(episode)[0]  # Legacy packages retain strict current-script lookup.
    if not isinstance(source, dict) or not source.get("path"):
        raise Refused("REFUSED — production package has no valid source beat-package path")
    root = (_source_base() / "cb-output").resolve()
    path = (_source_base() / str(source["path"])).resolve()
    if path.parent != root or not path.name.startswith(f"{episode}_") or not path.name.endswith("beat_package.json"):
        raise Refused("REFUSED — production source beat-package path is outside this episode")
    try:
        beats = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        raise Refused("REFUSED — production source beat package is unavailable") from exc
    signature = cb_lineage.beat_package_signature(beats)
    source_script = pkg.get("sourceScript") or {}
    if (beats.get("contentSignature") != signature or
            source.get("contentSignature") != signature or
            (pkg.get("inputSignature") or {}).get("inputs", {}).get("beatPackageDigest") != signature["digest"] or
            beats.get("sourceScript") != source_script):
        raise Refused("REFUSED — production source beat package changed after scene approval")
    report = cb_lineage.validate_beat_package_source_contract(beats)
    if not report["ok"]:
        raise Refused("REFUSED — production source beats have an invalid exact-event contract")
    current = SCRIPT_STORE.current(episode, required=True)
    if (current["scriptVersionId"] != source_script.get("scriptVersionId") and
            (current.get("changeScope") or {}).get("kind") == "dialogue-format-cleanup"):
        # A formatting-only pointer may preserve existing production, but only when
        # every word and speaker still matches the approved source in order.
        import cb_intake
        from cb_source_refresh import plan
        raw = (_source_base() / current["contentPath"]).read_text(encoding="utf-8")
        events = cb_intake.parse_script(raw, cb_intake._load_roster(), log=lambda *args: None)["events"]
        cb_intake._annotate_source_events(events, current["scriptVersionId"])
        try:
            plan(beats, events, current["scriptVersionId"])
        except (ValueError, cb_intake.Refused) as exc:
            raise Refused("REFUSED — script cleanup changed source words or beat boundaries") from exc
    return beats
...
def _fresh_validation(pkg, episode, target_shot_id=None):
    ...
    d = _validation_beat_package(pkg, episode)
    ...
    # Fresh WATCH validation must use the same hash-verified Audio1 placement as
    # the sealed provider request. DIRECT timings remain estimates after approval.
    from studio_approved_media_projection import watch_shot
    projected_shots = []
    for rec in current_shots:
        ledger = _ledger(pkg, rec.get("shotId"))
        approval = ledger.get("voiceApproval") or {}
        if approval.get("approved") and ledger.get("voPlacementPath"):
            rec = watch_shot(rec, ledger)
        projected_shots.append(rec)
    current_shots = projected_shots
    ...
    if target_shot_id:
        ...
        target_voice = (
            target_ledger.get("voiceApproval")
            or ((target_ledger.get("departmentWork") or {}).get("voice") or {}).get("approved")
            or {}
        )
        target_voice_approved = bool(target_voice.get("approved") or target_voice.get("outcome") == "approved")
        target_rec = next((rec for rec in pkg["shots"]
                           if rec.get("shotId") == target_shot_id), None)
        target_line_ids = {
            line.get("dialogueOccurrenceId")
            for line in (target_rec or {}).get("dialogueLines") or []
            if line.get("dialogueOccurrenceId")
        }
        source_line_ids = {
            cut.get("dialogueOccurrenceId")
            for beat in beats
            for cut in (beat.get("cuts") or [])
            if cut.get("dialogueOccurrenceId")
        }
        if (target_rec and target_rec.get("dialogueLines") and
                (not beats or not target_line_ids.issubset(source_line_ids))):
            # Scoped production amendments can legitimately leave the current
            # episode beat package with renamed/rehomed beat codes while this shot
            # already has occurrence-locked shot dialogue. Fresh WATCH validation still
            # needs a verbatim source ledger; use the current shot dialogue as that
            # temporary source instead of comparing against stale episode-wide beats and
            # falsely rejecting every line as non-verbatim.
            voice_cuts = []
            ...
            synthetic_codes = selected_codes or [target_rec.get("beatCode") or target_shot_id]
            beats = []
            for index, code in enumerate(synthetic_codes):
                cuts = voice_cuts if index == 0 else []
                beats.append({
                    "beatCode": code,
                    "sceneNumber": pkg.get("sceneNumber"),
                    "storyBeat": target_rec.get("purpose") or target_rec.get("visualPayoff") or target_shot_id,
                    "cuts": cuts,
                    "dialogueOccurrenceIds": [
                        cut["dialogueOccurrenceId"] for cut in cuts
                        if cut.get("dialogueOccurrenceId")
                    ],
                })
    ...
    if target_shot_id:
        if (target_voice_approved and target_rec and
                (target_rec.get("dialogueLines") or [])):
...
def _sealed_envelope(pkg, shot, led, imgs, anchor, candidates, fast, per,
    ...
    specialist = {}  # WATCH has no second creative authority.
    from studio_delivery_contract import require_aligned_timing
    try:
        ...
        raise Refused(str(exc)) from exc
    prompt_source = "direct-deterministic-compiler"
    ...
    env["learningScope"] = {"projectId": pkg.get("projectId") or "crystal-bears",
                            "episode": pkg.get("episode"), "scene": str(pkg.get("sceneNumber", ""))}
    try:
        ...
        for segment in env["executionPlan"]["segments"]:
            ...
            segment["promptAudit"] = audit
            from studio_prompt_quality import assess as assess_quality, require as require_quality
            segment['promptQuality'] = assess_quality(segment['promptDirectorSnapshot'])
            from studio_request_evidence import _write, digest
            _write(ROOT / 'cb-output/state/prompt-quality' / (digest(segment['promptQuality']) + '.json'),
                   {'snapshot': segment['promptDirectorSnapshot'], 'review': segment['promptQuality']})
            require_quality(segment['prompt'], segment['promptQuality'])
    except ValueError as exc:
        raise Refused(str(exc)) from exc
    tracked_report = _require_tracked_object_report(
        {**shot, "seedancePrompt": env['prompt']}, env['prompt'], env.get('references') or [])
    env['trackedProductionObjects'] = tracked_report
    for segment in env["executionPlan"]["segments"]:
        segment['trackedProductionObjects'] = _tracked_object_report(
            {**shot, "seedancePrompt": segment.get('prompt') or env['prompt']},
            segment.get('prompt') or env['prompt'],
            segment.get('references') or env.get('references') or [],
            archive=False)
    from studio_director_card import card
    import hashlib
    ...
    from studio_prompt_emission import record_shadow
    record_shadow(ROOT, 'native', env['productionRequest'])
    import hashlib
...
def _verify_envelope(auth):
    ...
    if h != auth.get("envelopeHash"):
        raise Refused("REFUSED — sealed-envelope integrity check failed; request a new "
                      "disclosure.")
    if env.get('productionRequest'):
        from studio_shot_request import ShotProductionRequest, native_envelope
        try:
            saved_request = ShotProductionRequest.load(env['productionRequest'])
            if saved_request.request_hash != native_envelope(env).request_hash:
                raise ValueError('Sealed production request does not match its execution plan')
        except ValueError as exc:
            raise Refused(str(exc)) from exc
    ...
    for segment in segments:
        from studio_prompt_quality import require as require_quality
        require_quality(segment['prompt'], segment.get('promptQuality'))
...
def _resolve_seedance_prompt(pkg, shot, scene=None, episode="Ep1", require_current_working=False):
    """Compile current DIRECT without department generation or prompt overrides."""
    from studio_approved_media_projection import watch_shot
    from studio_prompt_director import compile_native_source
    led = _ledger(pkg, shot['shotId'])
    source = watch_shot(shot, led)
    scene = str(scene if scene is not None else pkg.get('sceneNumber') or '')
    source = _with_effective_reference_slots(pkg, source, 'referenceSlots', scene, episode)
    anchor = _anchor_for(pkg, source)
    attachments = _provider_attachment_plan(source, 'referenceSlots', anchor, scene, episode, _characters_cfg())
    refs = _reference_records(source, [item['path'] for item in attachments])
    audio = {'path': led['voPath'], 'md5': _file_md5(led['voPath'])} if led.get('voPath') else {}
    try:
...
def seedance_working_status(scene, shot_id, episode="Ep1"):
    """Read-only current DIRECT compilation; no department prompt is a baseline."""
    pkg, _ = load_pkg(scene, episode)
    shot = _shot(pkg, shot_id)
    current, _ = _resolve_seedance_prompt(pkg, shot, scene, episode)
    return {"source": "direct-deterministic-compiler", "approvedPrompt": current,
            "currentPrompt": current, "isWorking": False, "savedAt": None}



def save_seedance_working(scene, shot_id, prompt_text, episode="Ep1", reviewed_by="Julian", log=print):
    """WATCH has no editable creative prompt. Preserve existing records."""
    raise Refused('DIRECTOR_REVISION_REQUIRED: edit and approve the action, camera or performance in DIRECT. '
                  'WATCH compiles those fields; it cannot save replacement provider prose.')
...
def save_watch_director_feedback(scene, shot_id, feedback, episode="Ep1",
    ...
    if (previous or {}).get("text") == note and _shot(pkg, shot_id).get("watchDirectorFeedbackApproved") == note:
        log(f"WATCH FEEDBACK CURRENT — {shot_id} (saved direction reused)")
        return previous
    if previous:
    ...
    # The current note is part of derived Director Card provenance. Direct Fire
    # must see a stale handoff until normal preparation has translated this note.
    _shot(pkg, shot_id)["watchDirectorFeedbackApproved"] = note
    _save(pkg, path)
    ...
    try:
        ...
        key = hashlib.sha256(json.dumps({
            "episode": episode, "scene": str(scene), "shot": shot_id,
            "feedback": ledger["watchDirectorFeedback"],
        }, sort_keys=True).encode()).hexdigest()
        cb_learning.capture_evidence(
            "retaken", note, episode=episode, scene=str(scene), shot=shot_id,
            sourceVersion=pkg.get("revision"), role="Animation", category="creative",
            scope="shot", capturedBy=reviewed_by, decisionKey=key,
            context=json.dumps({"characters": shot.get("charactersInFrame") or [],
                                "request": "bounded-watch-feedback"}),
            assetPointers=[ledger["approvedTake"]] if ledger.get("approvedTake") else [])
    except Exception as exc:
        log(f"LEARNING CAPTURE WARNING — feedback saved: {exc}")
...
def _animation_prompt_contract_report(shot, direction):
    ...
    if not authoring.get("productionReady", authoring["status"] == "ready"):
        errors.extend(authoring.get("productionErrors") or authoring["repairActions"])
...
def _animation_preflight_summary(shot, direction):
    ...
    if authoring.get("status") != "ready":
        findings.extend({
            "severity": "WARNING" if authoring.get("productionReady") else "FATAL",
            "rule": "seedance-authoring",
            "message": action,
...
def _seedance_pipeline_task(shot, specialist, attached_contract):
    ...
    audio = specialist.get("audioContract") or (
        emission.STANDARD_DIALOGUE_AUDIO_AUTHORITY + "\n" +
...
def check_seedance_structure(scene, shot_id, episode="Ep1", log=print):
    ...
    if not seedance_contract.get("productionReady", seedance_contract["status"] == "ready"):
        blockers.extend(seedance_contract.get("productionErrors") or seedance_contract["repairActions"])
    elif seedance_contract["status"] != "ready":
        warnings.extend(seedance_contract["repairActions"])
    ...
    for s in sentences:
        ...
        if any(marker in normalized for marker in (
                ...
                "exclude its background people and composition",
                "hold 2 0s hold the full team beach arrival with keen alive and squeaky safe",
                "no extra voices",
                "during approved laughter or giggling in audio1 animate only the characters audibly participating in that exact interval",
                "no music",
                "no ambience",
                "no sfx",
                "in every hold and final landing preserve subtle breathing natural blink timing focused eyes and a precise asymmetric micro expression",
                "the beat owner looks toward the story target the witness looks toward the beat owner or visible evidence",
                "no vacant forward stare unfocused eyes frozen smile mannequin stillness or generic camera facing expression",
...
def _prompt_contract_is_exact(contract):
    ...
    for name in ("promptStandard", "promptSections"):
        if name in contract:
            names.append(name)
...
def _current_prompt_contract(pkg, shot, artifact_type):
    ...
    if artifact_type == "keyframe":
        # keyframe_shot sends this same current DIRECT-card compilation. A stored
        # specialist prompt may use an older brief schema and must not shadow the
        # prompt that the paid route will actually submit.
        prompt = _resolve_keyframe_prompt(pkg, shot)
...
def _returned_origin(ledger):
    """Read historical evidence without making a corrupt record break media playback.

    The outer seal binds the historical segment snapshots and reference paths too.
    Re-projecting history with today's request adapter cannot verify that binding.
    """
    batch = ledger.get("batch") or {}
    envelope = batch.get("envelope") or {}
    from studio_shot_request import origin, originating_review_context
    expected_hash = batch.get("envelopeHash")
    # Match the existing fire-time serializer exactly, including its whitespace.
    actual_hash = hashlib.sha256(json.dumps(
        envelope, sort_keys=True, ensure_ascii=False).encode()).hexdigest()
    integrity = {"verified": bool(expected_hash and expected_hash == actual_hash),
                 "expectedEnvelopeHash": expected_hash, "actualEnvelopeHash": actual_hash}
    result = {"batchId": batch.get("batchId") or ledger.get("batchId"),
              "envelopeHash": expected_hash, "originIntegrity": integrity,
              "directorCardRevision": None, "segments": [],
    ...
    def failed(reason):
        lineage = {"status": "origin-integrity-failed", "originatingRequestHash": None}
        integrity.update(verified=False, reason=reason)
        result.update(status=lineage["status"], requestLineage=lineage,
                      originatingProduction={**lineage, "observed": False, "approved": False,
                          "reviewScope": "Originating evidence unavailable: integrity failed"})
        result["reviewScope"]["planned"] = "unavailable; originating production integrity failed"
        return result

    if expected_hash and expected_hash != actual_hash:
        return failed("Stored envelope hash does not match the historical batch")
    if not isinstance(envelope, dict):
        return failed("Historical envelope is not a structured production record")
    execution_plan = envelope.get("executionPlan") or {}
    if not isinstance(execution_plan, dict):
        return failed("Historical execution plan is not structured")
    segments = execution_plan.get("segments") or []
    if not isinstance(segments, list) or any(not isinstance(s, dict) for s in segments):
        return failed("Historical execution segments are not structured")
    try:
        lineage = origin(envelope)
        production = originating_review_context(envelope)
    except (TypeError, ValueError) as exc:
        return failed(str(exc))
    if not expected_hash:
        # An internally valid nested request cannot authenticate the surrounding
        # historical reference paths and specialist snapshots without this seal.
        lineage = {"status": "legacy-unverified", "originatingRequestHash": None}
        production = {**lineage, "observed": False, "approved": False,
                      "reviewScope": "Historical envelope has no seal; source attribution is unverified"}
        integrity["reason"] = "Historical envelope has no stored hash"
        result["reviewScope"]["planned"] = "historical recorded plan; unverified without an envelope seal"
    result.update(status=lineage["status"], requestLineage=lineage,
                  originatingProduction=production,
...
def _latest_media_review(ledger, artifact_type):
    ...
    return {
        ...
        "failureClass": output.get("failureClass", "unverified"),
        "failureClassEvidence": output.get("failureClassEvidence"),
...
def fire_shot(scene, shot_id, episode="Ep1", candidates=DEFAULT_CANDIDATES, fast=False,
    ...
    if protected_comparison:
        if led.get("status") != "approved" or not led.get("approvedTake"):
            raise Refused(
                f"REFUSED — {shot_id} needs an approved take before a protected prompt comparison")
        comparison_work = led.get("comparisonWork") or {}
    ...
    auth_field = "pendingComparisonSpendAuth" if protected_comparison else "pendingSpendAuth"
    batch_field = "comparisonBatch" if protected_comparison else "batch"
    spend_shot_id = f"{shot_id}.prompt-comparison" if protected_comparison else shot_id
    existing_auth = led.get(auth_field) or {}
    existing_batch = led.get(batch_field) or {}
    existing_envelope = (
    ...
    from studio_approved_media_projection import watch_shot
    rule_shot = watch_shot(shot, led)
    animation_direction = native_rule_inputs(rule_shot, resolved_prompt)
    _require_engine_rules(pkg, rule_shot, animation_direction, cinematography={})
    candidates = max(1, min(MAX_CANDIDATES, int(candidates)))
    if protected_comparison:
        candidates = 1
    elif led.get("status") == "approved":
        raise Refused(f"REFUSED — {shot_id} is already approved; reject it first to re-fire")
    _require_stage_contract_keyframe(shot, led, pkg)
    ...
    shot["seedancePrompt"] = emission.ensure_standard_audio_template(
    ...
    try:
        import studio_tracked_objects
        prompt = studio_tracked_objects.apply_provider_clauses(prompt, shot)
    except Exception:
        pass
    if prompt != shot.get("seedancePrompt"):
        shot = {**shot, "seedancePrompt": prompt}
    anchor = _anchor_for(pkg, shot)
    characters_cfg = _characters_cfg()
    ...
    if batch and batch.get("status") == "generating":
        ...
        try:
            cb_db.claim_spend_authorization(
                HERE.parent, spend_token, episode, scene, spend_shot_id, binding,
                batch["envelopeHash"], batch["batchId"])
        except cb_db.SpendConflict as exc:
            raise Refused(f"REFUSED — {exc}") from exc
    else:
        if not protected_comparison and led.get("status") == "candidates-pending":
        ...
        disclosure = {"shotId": shot_id, "candidateCount": candidates,
                       ...
                       "trackedProductionObjects": {
                           "status": (envelope.get("trackedProductionObjects") or {}).get("status"),
                           "objectResolutionHash": (envelope.get("trackedProductionObjects") or {}).get("objectResolutionHash"),
                           "evidencePath": (envelope.get("trackedProductionObjects") or {}).get("evidencePath"),
                           "objects": [item.get("id") for item in (envelope.get("trackedProductionObjects") or {}).get("objects") or []],
                       },
        ...
        log("  trackedProductionObjects: " + json.dumps(disclosure["trackedProductionObjects"], ensure_ascii=False))
        ...
        if spend_token is None:
            ...
            raise Refused("REFUSED — SPEND NOT APPROVED. A single-use spend token has been "
                          ...
                          f"--spend-token {led[auth_field]['token']} "
        ...
        try:
            ...
            raise Refused(f"REFUSED — {exc}") from exc
        led[auth_field] = None                              # claimed now; consumed on completion
        pv = auth["disclosure"]["promptVersion"]
        if not protected_comparison:
        ...
        batch = {"token": spend_token, "bindingHash": binding,
                 ...
                 "audioProvenance": ({
                     "policyVersion": "provider-final-mix-v1",
                     ...
                     "providerOutputRole": "final-mix-awaiting-human-review",
                     "finalFilmDialogue": "audio1-conditioned-human-reviewed",
                 } if cb_audio_authority.spoken_dialogue_lines(shot) and led.get("voPath") else None)}
        led[batch_field] = batch
        if protected_comparison:
            led["comparisonWork"] = {
                "status": "generating",
                "batchId": batch_id,
                "sourcePath": led.get("approvedTake"),
                "sourceSha256": _sha256_file(led["approvedTake"]),
                "prompt": prompt,
    ...
    for i in range(1, batch["expected"] + 1):
        out = (MEDIA / f"{episode}_{shot_id}_compare_{batch['batchId']}_c{i}.mp4"
               if protected_comparison else MEDIA / f"{episode}_{shot_id}_c{i}.mp4")
        try:
            resume_output = pathlib.Path(str(out) + "." + batch["batchId"])
            claim = cb_db.claim_candidate(
                HERE.parent, batch["token"], i,
                f"{os.getpid()}:{threading.get_ident()}",
                resume_existing=(cb_provider_jobs.resumable(resume_output) or any(
                    cb_provider_jobs.resumable(pathlib.Path(str(
                        MEDIA / "transport" / batch["batchId"] / f"c{i}" /
                        f"segment_{seg['segmentIndex']}.mp4") + "." + batch["batchId"]))
            ...
            for segment in segments:
                ...
                if segment_count > 1:
                    segment_claim = cb_db.claim_candidate_segment(
                        ...
                        f"{os.getpid()}:{threading.get_ident()}",
                        resume_existing=cb_provider_jobs.resumable(
                            pathlib.Path(str(segment_out) + "." + batch["batchId"])))
                if segment_claim["action"] == "completed":
                    ...
                    _submit_seedance_provider(
                        ...
                        direction_evidence={'directorCardRevision': envelope.get('directorCardRevision'),
                                            'productionRequest': envelope.get('productionRequest'),
                                            'mediaBindings': {'image': segment.get('references') or [],
                                                              'audio': [segment['audio']] if segment.get('audio') else [],
                                                              'video': segment.get('videoReferences') or []},
                                            'envelopeHash': batch.get('envelopeHash'),
                                            'promptQuality': segment.get('promptQuality'),
                                            'segmentIndex': segment_index},
                        **generate_kwargs)
                    if segment_count > 1:
    ...
    paths = [
        str(MEDIA / f"{episode}_{shot_id}_compare_{batch['batchId']}_c{i}.mp4")
        if protected_comparison else str(MEDIA / f"{episode}_{shot_id}_c{i}.mp4")
        for i in sorted(batch["done"])
    ]
    if protected_comparison:
        source = pathlib.Path(led.get("approvedTake") or "")
        work = led.get("comparisonWork") or {}
        if (led.get("status") != "approved" or not source.is_file() or
                _sha256_file(source) != work.get("sourceSha256")):
            raise Refused(
                f"REFUSED — {shot_id}'s approved source changed while the comparison rendered")
        batch["candidatePaths"] = paths
        work.update({
            "status": "candidate-pending", "candidatePath": paths[0],
            "candidateSha256": _sha256_file(paths[0]), "completedAt": _now(),
        })
        led["comparisonWork"] = work
    else:
    ...
    try:
        ...
        raise Refused(f"REFUSED — {exc}") from exc
    if protected_comparison:
        log(f"COMPARISON READY — {shot_id}: one alternative rendered from the current "
            "prompt; the approved take remains unchanged until Julian decides.")
    else:
...
def import_animation_candidate(scene, shot_id, upload_path, episode="Ep1", log=print):
    ...
    _require_stage_contract_keyframe(shot, ledger, pkg)
...
def _restore_approved_voice_for_review(shot, ledger, candidate_path, batch_id, candidate):
    """Preserve the returned mix for review, never overlay or strip its audio.

    Historical function name retained for callers. Audio1 conditions the render;
    waveform identity and lip sync are not guaranteed and require human review.
    """
    ...
    provider_guide = pathlib.Path(str(candidate_path) + ".provider-guide.mp4")
    if not provider_guide.exists():
        shutil.copy2(candidate_path, provider_guide)
    return {
        "candidate": candidate,
...
def _video_edit_prompt(shot, correction, start_sec, end_sec, has_audio):
    audio = (
        ...
        "\nFor this edit, preserve the source video's existing sound bed outside the "
        "correction window. Do not regenerate or layer a second dialogue track. "
        if has_audio else
...
def edit_shot(scene, shot_id, correction, start_sec, end_sec, episode="Ep1",
    ...
    from studio_final_direction import model_options
    final, coherence = review_edit(review_source, lambda system, data: cb_llm.structured_with_repair(
        ...
        **model_options()), review_plan_first=True)
    from studio_request_evidence import _write
    ...
    from studio_shot_request import project as production_request
    edit_request = production_request('TARGETED_EDIT',
        scope={'projectId': pkg.get('projectId') or 'crystal-bears', 'episode': episode,
               'scene': str(scene), 'shotId': shot_id}, source=pkg.get('sourceScript'),
        direction=shot.get('directorCard') or {}, dialogue=shot.get('dialogueLines') or [],
        refs=final.get('references') or [], audio={'sha256': binding_payload['audioSha256']},
        prompt=prompt, settings=contract, edit_scope=edit_scope).record()
    binding_payload['productionRequest'] = edit_request
    ...
    ledger["editWork"] = {
        ...
        "productionRequest": edit_request,
    ...
    try:
        _submit_seedance_provider(
            ...
            direction_evidence={'productionRequest': edit_request, 'editScope': edit_scope,
                                'mediaBindings': {'video':[{'sha256':source_hash}],
                                                  'audio':[{'sha256':binding_payload['audioSha256']}] if has_audio else []}},
...
def approve_shot_comparison(scene, shot_id, episode="Ep1",
                            reviewed_by="Julian", log=print):
    """Promote a protected prompt comparison only after an explicit human decision."""
    pkg, package_path = load_pkg(scene, episode)
    shot = _shot(pkg, shot_id)
    ledger = _ledger(pkg, shot_id)
    work = ledger.get("comparisonWork") or {}
    candidate = pathlib.Path(work.get("candidatePath") or "")
    source = pathlib.Path(work.get("sourcePath") or "")
    if ledger.get("status") != "approved" or not ledger.get("approvedTake"):
        raise Refused(f"REFUSED — {shot_id} no longer has a protected approved source")
    if work.get("status") != "candidate-pending" or not candidate.is_file():
        raise Refused(f"REFUSED — {shot_id} has no prompt comparison awaiting approval")
    if _sha256_file(candidate) != work.get("candidateSha256"):
        raise Refused("REFUSED — prompt-comparison bytes changed after generation")
    if (str(source.resolve()) != str(pathlib.Path(ledger["approvedTake"]).resolve()) or
            not source.is_file() or _sha256_file(source) != work.get("sourceSha256")):
        raise Refused("REFUSED — the protected source changed after the comparison began")
    comparison_batch = ledger.get("comparisonBatch") or {}
    comparison_signature = comparison_batch.get("inputSignature")
    if (not isinstance(comparison_signature, dict) or
            comparison_batch.get("batchId") != work.get("batchId")):
        raise Refused(
            "REFUSED — the prompt comparison has no matching signed generation inputs")
    previous = {
        "status": "superseded-by-approved-comparison",
        "path": str(source), "sha256": work["sourceSha256"],
        "approval": ledger.get("approval"),
        "harvestFrame": ledger.get("harvestFrame"),
        "comparisonBatchId": work.get("batchId"), "at": _now(),
    }
    ledger.setdefault("comparisonHistory", []).append(previous)
    harvest = MEDIA / f"{episode}_{shot_id}_final_frame.png"
    cb_gen.last_frame(candidate, out=str(harvest))
    ledger.update({
        "status": "approved", "approvedTake": str(candidate),
        "approvedCandidate": "prompt-comparison",
        "harvestFrame": str(harvest),
        "approval": {
            "approved": True, "source": "protected-prompt-comparison",
            "reviewed_by": reviewed_by, "at": _now(),
            "packageRevision": pkg.get("revision"),
            "inputSignature": comparison_signature,
            "contentHash": _sha256_file(candidate),
            "harvestHash": _sha256_file(harvest),
            "batchId": comparison_batch.get("batchId"),
            "promptHash": comparison_batch.get("promptHash"),
        },
        "comparisonWork": None,
    })
    _candidate_review(shot, str(candidate), work.get("batchId") or "prompt-comparison", 1)
    _save(pkg, package_path)
    log(f"COMPARISON APPROVED — {shot_id}; prior approved take remains in comparison history")
    return str(candidate)


def reject_shot_comparison(scene, shot_id, episode="Ep1",
                           reviewed_by="Julian", log=print):
    """Archive one comparison candidate while leaving the approved source untouched."""
    pkg, package_path = load_pkg(scene, episode)
    ledger = _ledger(pkg, shot_id)
    work = ledger.get("comparisonWork") or {}
    candidate = pathlib.Path(work.get("candidatePath") or "")
    source = pathlib.Path(work.get("sourcePath") or "")
    if work.get("status") != "candidate-pending" or not candidate.is_file():
        raise Refused(f"REFUSED — {shot_id} has no prompt comparison awaiting rejection")
    if (ledger.get("status") != "approved" or not source.is_file() or
            str(source.resolve()) != str(pathlib.Path(ledger.get("approvedTake") or "").resolve()) or
            _sha256_file(source) != work.get("sourceSha256")):
        raise Refused("REFUSED — the protected approved source is no longer current")
    approval = ledger.get("approval") or {}
    source_batch = ledger.get("batch") or {}
    source_signature = approval.get("inputSignature")
    source_batch_id = str(approval.get("batchId") or ledger.get("batchId") or "")
    if not isinstance(source_signature, dict):
        if (source_batch.get("batchId") != source_batch_id or
                not isinstance(source_batch.get("inputSignature"), dict)):
            raise Refused(
                "REFUSED — the protected approved source has no matching signed generation inputs")
        source_signature = source_batch["inputSignature"]
    harvest = pathlib.Path(ledger.get("harvestFrame") or "")
    if not harvest.is_file():
        raise Refused("REFUSED — the protected approved source has no final-frame handoff")
    archive = (MEDIA / "archive" / "prompt_comparisons" /
               f"{episode}_{shot_id}_{uuid.uuid4().hex[:8]}")
    archive.mkdir(parents=True, exist_ok=True)
    archived = archive / candidate.name
    shutil.move(str(candidate), archived)
    sidecars = []
    for suffix in (".review.json", ".gen.json"):
        sidecar = pathlib.Path(str(candidate) + suffix)
        if sidecar.is_file():
            destination = archive / sidecar.name
            shutil.move(str(sidecar), destination)
            sidecars.append(str(destination))
    ledger.setdefault("comparisonHistory", []).append({
        **work, "status": "not-selected", "candidatePath": str(archived),
        "archivedSidecars": sidecars, "reviewedBy": reviewed_by,
        "reviewedAt": _now(),
    })
    ledger["comparisonWork"] = None
    approval.update({
        "approved": True,
        "packageRevision": pkg.get("revision"),
        "inputSignature": source_signature,
        "contentHash": _sha256_file(source),
        "harvestHash": _sha256_file(harvest),
        "batchId": source_batch_id,
        "keptCurrentAt": _now(),
        "keptCurrentBy": reviewed_by,
        "keptCurrentVia": "protected-prompt-comparison-not-selected",
    })
    ledger["approval"] = approval
    _save(pkg, package_path)
    log(f"COMPARISON NOT SELECTED — {shot_id}; approved take remains unchanged")
    return str(archived)
...
def next_shot(scene, episode="Ep1", candidates=DEFAULT_CANDIDATES, fast=False,
    ...
    pkg, path = load_pkg(scene, episode)
    _require_valid(pkg)
    for s in production_contracts.active_shots(pkg):
        led = _ledger(pkg, s["shotId"])
...
def recover_approved_shot(scene, shot_id, episode="Ep1", log=print):
    ...
    with cb_db.scene_lease(HERE.parent, episode, str(scene),
        ...
        source_batch = led.get("batch") or {}
        if (source_batch.get("batchId") != batch_id or
                not isinstance(source_batch.get("inputSignature"), dict)):
            raise Refused(
                f"REFUSED — {shot_id} recovery needs the matching signed provider batch")
        ...
        led.update({
            ...
            "approval": {
                ...
                "batchId": batch_id,
                "packageRevision": pkg.get("revision"),
                "inputSignature": source_batch["inputSignature"],
                "recoveredAt": recovered_at,
            },
        })
        _save(pkg, path)
...
def reject_shot(scene, shot_id, correction, category="other", episode="Ep1",
    ...
    one-sentence correction and its failure category on record. The next fire is always a
    controlled reroll of the unchanged package. Repeated rejection is advisory only."""
    pkg, path = load_pkg(scene, episode)
    shot = _shot(pkg, shot_id)
    led = _ledger(pkg, shot_id)
    if led.get("status") != "candidates-pending" or not led.get("candidatePaths"):
    ...
    # Rejection consumes the old disclosure. Leaving it attached makes a later
    # reroll look like an active or stale spend operation and blocks the workflow.
    led.update({"batchAttempts": attempts, "candidatePaths": None,
                "batchId": None, "pendingSpendAuth": None})
    led["status"] = "designed"
    led["rejectionAdvisory"] = {
        "batchAttempts": attempts,
        "message": "Repeated rejected batches are advisory only; revise DIRECT or prepare again.",
        "at": _now(),
    }
    _save(pkg, path)
    log(f"REJECTED — {shot_id} batch archived ({attempts} failed batches). "
...
def override_model_limited(scene, shot_id, reason, episode="Ep1",
                           ...
                           log=print):
    """Legacy migration: audit reopening of an old model-limited shot.
...
def _scene_post_sources(pkg, scene=None, episode=None):
    ...
    for s in pkg["shots"]:
        ...
        if led.get("status") == "approved" and led.get("approvedTake"):
            ...
            lineage = origin(batch.get('envelope') or {}) if led['approvedTake'] in (batch.get('candidatePaths') or []) else origin({})
            source_by_id[s["shotId"]] = {
                ...
                "audioProvenance": led.get("audioProvenance"), 'requestLineage':lineage}
        else:
...
def fire_credits(episode, token):
    """Credits use their own sealed episode artifact, without fabricating scene approvals."""
    import cb_credits
    return cb_credits.fire(episode, token)


def render_credits_job(episode, job, resume_existing=False):
    import cb_credits
    return cb_credits.run_worker(episode, job, resume_existing)
...
# Explicit composition root: each public operation names its production policy service.
# The compatibility names below remain stable for workers and existing saved commands.
import cb_safety as _cb_safety
_production_policy = _cb_safety.create_policy(sys.modules[__name__])
_resolve_scenelook_prompt = _production_policy._resolve_scenelook_prompt
scenelook_status = _production_policy.scenelook_status
_scenelook_record_input_signature = _production_policy._scenelook_record_input_signature
approved_look_prompt = _production_policy.approved_look_prompt
generate_scenelook_plate = _production_policy.generate_scenelook_plate
approve_scenelook = _production_policy.approve_scenelook
select_scenelook_source = _production_policy.select_scenelook_source
department_status = _production_policy.department_status
prepare_department = _production_policy.prepare_department
save_department_candidate = _production_policy.save_department_candidate
decide_department = _production_policy.decide_department
_current_department_output = _production_policy._current_department_output
_require_approved_department_output = _production_policy._require_approved_department_output
_approved_department_output = _production_policy._approved_department_output
_department_input_signature = _production_policy._department_input_signature
_department_record_status = _production_policy._department_record_status
_resolve_keyframe_prompt = _production_policy._resolve_keyframe_prompt
_keyframe_input_signature = _production_policy._keyframe_input_signature
_keyframe_stage_contract_report = _production_policy._keyframe_stage_contract_report
_approved_voice_lines = _production_policy._approved_voice_lines
_voice_input_signature = _production_policy._voice_input_signature
_voice_approval_status = _production_policy._voice_approval_status
_voice_signature = _production_policy._voice_signature
_animation_input_signature = _production_policy._animation_input_signature
_animation_generation_signature = _production_policy._animation_generation_signature
_animation_approval_status = _production_policy._animation_approval_status
_external_import_input_signature = _production_policy._external_import_input_signature
voice_shot = _production_policy.voice_shot
approve_voice = _production_policy.approve_voice
reject_voice = _production_policy.reject_voice
restore_previous_voice_take = _production_policy.restore_previous_voice_take
keyframe_shot = _production_policy.keyframe_shot
select_keyframe_source = _production_policy.select_keyframe_source
rescreen_keyframe_conformance = _production_policy.rescreen_keyframe_conformance
approve_keyframe = _production_policy.approve_keyframe
reassess_keyframe = _production_policy.reassess_keyframe
_keyframe_record_input_signature = _production_policy._keyframe_record_input_signature
_keyframe_record_status = _production_policy._keyframe_record_status
_anchor_for = _production_policy._anchor_for
_resolve_seedance_prompt = _production_policy._resolve_seedance_prompt
_approved_seedance_prompt = _production_policy._approved_seedance_prompt
check_seedance_structure = _production_policy.check_seedance_structure
fire_shot = _production_policy.fire_shot
next_shot = _production_policy.next_shot
approve_shot = _production_policy.approve_shot
stitch_scene = _production_policy.stitch_scene
import cb_transactions as _cb_transactions
_runtime = sys.modules[__name__]
generate_scenelook_plate = _cb_transactions.protect(_runtime, "generate_scenelook_plate", generate_scenelook_plate)
approve_scenelook = _cb_transactions.protect(_runtime, "approve_scenelook", approve_scenelook)
reject_scenelook = _cb_transactions.protect(_runtime, "reject_scenelook", reject_scenelook)
select_scenelook_source = _cb_transactions.protect(_runtime, "select_scenelook_source", select_scenelook_source)
prepare_department = _cb_transactions.protect(_runtime, "prepare_department", prepare_department)
review_voice_source_boundaries = _cb_transactions.protect(
    _runtime, "review_voice_source_boundaries", review_voice_source_boundaries)
prepare_voice_cues = _cb_transactions.protect(_runtime, "prepare_voice_cues", prepare_voice_cues)
save_department_candidate = _cb_transactions.protect(_runtime, "save_department_candidate", save_department_candidate)
decide_department = _cb_transactions.protect(_runtime, "decide_department", decide_department)
save_voice_working = _cb_transactions.protect(_runtime, "save_voice_working", save_voice_working)
restore_voice_working = _cb_transactions.protect(_runtime, "restore_voice_working", restore_voice_working)
voice_shot = _cb_transactions.protect(_runtime, "voice_shot", voice_shot)
voice_scene = _cb_transactions.protect(_runtime, "voice_scene", voice_scene)
regen_voice_shot = _cb_transactions.protect(_runtime, "regen_voice_shot", regen_voice_shot)
approve_voice = _cb_transactions.protect(_runtime, "approve_voice", approve_voice)
reject_voice = _cb_transactions.protect(_runtime, "reject_voice", reject_voice)
restore_previous_voice_take = _cb_transactions.protect(_runtime, "restore_previous_voice_take", restore_previous_voice_take)
animatic_scene = _cb_transactions.protect(_runtime, "animatic_scene", animatic_scene)
build_keyframe = _cb_transactions.protect(_runtime, "build_keyframe", build_keyframe)
generate_pose_reference = _cb_transactions.protect(_runtime, "generate_pose_reference", generate_pose_reference)
select_pose_reference_source = _cb_transactions.protect(_runtime, "select_pose_reference_source", select_pose_reference_source)
review_pose_reference = _cb_transactions.protect(_runtime, "review_pose_reference", review_pose_reference)
qualify_pose_reference = _cb_transactions.protect(_runtime, "qualify_pose_reference", qualify_pose_reference)
reuse_qualified_pose = _cb_transactions.protect(_runtime, "reuse_qualified_pose", reuse_qualified_pose)
approve_pose_reference = _cb_transactions.protect(_runtime, "approve_pose_reference", approve_pose_reference)
reject_pose_reference = _cb_transactions.protect(_runtime, "reject_pose_reference", reject_pose_reference)
keyframe_shot = _cb_transactions.protect(_runtime, "keyframe_shot", keyframe_shot)
select_keyframe_source = _cb_transactions.protect(_runtime, "select_keyframe_source", select_keyframe_source)
approve_keyframe = _cb_transactions.protect(_runtime, "approve_keyframe", approve_keyframe)
reject_keyframe = _cb_transactions.protect(_runtime, "reject_keyframe", reject_keyframe)
save_seedance_working = _cb_transactions.protect(_runtime, "save_seedance_working", save_seedance_working)
restore_seedance_working = _cb_transactions.protect(_runtime, "restore_seedance_working", restore_seedance_working)
fire_shot = _cb_transactions.protect(_runtime, "fire_shot", fire_shot)
next_shot = _cb_transactions.protect(_runtime, "next_shot", next_shot)
approve_shot = _cb_transactions.protect(_runtime, "approve_shot", approve_shot)
reject_shot = _cb_transactions.protect(_runtime, "reject_shot", reject_shot)
edit_shot = _cb_transactions.protect(_runtime, "edit_shot", edit_shot)
approve_shot_edit = _cb_transactions.protect(_runtime, "approve_shot_edit", approve_shot_edit)
reject_shot_edit = _cb_transactions.protect(_runtime, "reject_shot_edit", reject_shot_edit)
approve_shot_comparison = _cb_transactions.protect(_runtime, "approve_shot_comparison", approve_shot_comparison)
reject_shot_comparison = _cb_transactions.protect(_runtime, "reject_shot_comparison", reject_shot_comparison)
stitch_scene = _cb_transactions.protect(_runtime, "stitch_scene", stitch_scene)
reopen_approved_shot = _cb_transactions.protect(_runtime, "reopen_approved_shot", reopen_approved_shot)
import_animation_candidate = _cb_transactions.protect(_runtime, "import_animation_candidate", import_animation_candidate)
import_approved_take = _cb_transactions.protect(_runtime, "import_approved_take", import_approved_take)
recover_approved_shot = _cb_transactions.protect(_runtime, "recover_approved_shot", recover_approved_shot)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if cmd == "voice":
        ...
        elif cmd == "prepare-voice":
            prepare_voice_cues(pos[0], pos[1], ep(2))
        elif cmd == "save-voice":
            ...
            fire_shot(pos[0], pos[1], ep(2), candidates=flags["candidates"],
                       ...
                       comparison_model_id=flags["comparison_model"],
                       comparison_run_id=flags["comparison_run_id"])
        elif cmd == "compare-fire":
            fire_shot(pos[0], pos[1], ep(2), candidates=1,
                       spend_token=flags["spend_token"], dry_run=flags["dry_run"],
                       protected_comparison=True)
        ...
        elif cmd == "approve-comparison":
            approve_shot_comparison(pos[0], pos[1], ep(2))
        elif cmd == "reject-comparison":
            reject_shot_comparison(pos[0], pos[1], ep(2))
        elif cmd == "stitch":
        ...
        print(str(e))
        print("STUDIO_OUTCOME " + json.dumps({"outcome": production_contracts.command_outcome(1, cmd, [str(e)])}))
        sys.exit(1)
