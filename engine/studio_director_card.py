...
COVERAGE_KIND_ALIASES = {'environment': 'world_texture', 'establish': 'establishing', 'payoff': 'landing',
                         ...
                         'opening': 'establishing', 'button': 'landing'}
...
# The contract's own short words for the first three (20 Sep 2026: the Director used them
# and was refused; the check now accepts what the contract says).
ATTENTION_ALIASES = {'thought': 'character thought', 'performance': 'character performance',
                     'action': 'physical action'}


def canonical_attention(value):
    if value is None:
        return None
    key = str(value).strip().lower()
    return ATTENTION_ALIASES.get(key, key)
...
STORY CONTEXT FIRST. Before planning a unit, know what happened immediately before, what the audience already knows, what the characters know and do not know, the current emotional and relationship state, location and established geography, current and planted props, weather and world state, the current pace and the previous unit's rhythm, what this scene sets up and what the next beat needs. Preserve the script, approved canon, current production state and current DIRECT authority. Invent expressive staging and cinematic execution within those boundaries, never new story facts.
...
VOCABULARY, NEVER QUOTAS: hold, establishing, master, two_shot, pov, cut_in, insert, reaction, reveal, world_texture, match_on_action, transition, landing — exactly these words. No unit must contain any particular type.
...
OPENING AND LANDING. Every unit enters deliberately (world, detail, movement, reaction, character, object, geography, a sound-motivated image; not automatically the principal character) and ends at a usable editorial boundary. A continuing action may bridge units; do not force a mini-payoff onto every provider clip. The scene owns the emotional landing. A generation never merely runs out of seconds.
...
CAMERA AND LIGHT ARE EMOTIONAL DECISIONS, NEVER SPECS. Begin with what the audience must feel and notice, then the project's visual language. Choose a lens, stated in millimetres where meaningful, for perspective, camera distance, geography and intimacy, not a fixed emotion lookup. A quiet wide or fast telephoto can be expressive; explain this particular choice. A move needs a story trigger and a finish; lock the camera when performance or stillness carries the beat. A locked camera is a choice about stillness, never a default. Motivate light from the established world and shape attention through contrast, colour and composition; soft or flat light is valid when intentional. Preserve the show's supplied character camera grammar. Write angle, lens, movement, focus, light, composition, atmosphere and time treatment in the view's cinematography record; framing remains in framing. The compiler carries these authored fields to the provider's Camera line, while reference tagging binds character names. Optional fields are not a requirement to decorate every view.
HOW TO WRITE IT. Inside each view's cinematography record set kind (the vocabulary above), motivation (PLACE, FEEL, SEE, KNOW, REACT, LAUGH, FEAR, MOVE, BREATHE, REVEAL, SOUND, CONTRAST), cutTiming (before, on, after), actionPhase for physical chains only, exactly one of preparation, contact, completion, consequence, reaction (null when the view is not part of a chain), functions for world texture, and attention (one of the priorities named above, exactly as written). A hold is a view whose kind is hold, whose motivation is null (motivations belong to cuts, never to a hold) and whose cutReason says why staying is stronger. Record the scene's entrance, function and mode on sceneCoverage. Reaction views name the listener as viewpointOwner with listenerReaction; world-texture views list no character entity. Keep cutReason, cutTo, staging, action, performance, timing, startState and endState exact as before.
...
class CoverageView(Decision):
    ...
    cinematography: dict = Field(default_factory=dict, description="Per-view optional pattern, dramatic owner, emotional action, angle, lens, movement with trigger and finish, focus, light, composition, atmosphere and time treatment; no quotas. Numeric intervals remain in timing.")
...
def coverage_issues(view, previous=None):
    ...
    if attention is not None and canonical_attention(attention) not in ATTENTION_PRIORITIES:
