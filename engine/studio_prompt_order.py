...
def purpose_first(prompt):
    ...
    def rank(section):
        ...
        if 'audience purpose' in heading:return -1
        ...
        if any(x in heading for x in ('shot sequence','timed action','stage sequence','camera consciousness','living performance','coverage staging','action ownership','directed nonverbal')):return 1
...
def compile_order(prompt, direction):
    if '[Audience Purpose]' not in prompt:
