...
KNOWN GAP, NAMED HONESTLY (found 2026-07-22, chasing the new keyframe spend-token seal
through this file's own fixture — see _seed_cinematography's own docstring): keyframe_shot
has required an APPROVED Cinematography Direction since 2026-07-19's department-gate
hardening, and this file's own minimal synthetic shot (a handful of fields, not the real
cb_engine.Shot schema) lacks what _cinematography_dependency_context needs to build one
(e.g. "purpose") — a real, pre-existing gap independent of tonight's own seal, this file
never actually reached step 2 onward. This file is UNTRACKED and not collected by
`pytest -q` (it has no `def test_*` functions — its own `main()` only runs under
`if __name__ == "__main__"`), so it is not part of the load-bearing, currently-green
automated suite; two real, standalone improvements were made here tonight regardless (a
correct live-storyboard-md5 fixture value, and the keyframe spend-token seal's own two-phase
call pattern) rather than a full rebuild of this file's shot fixture to the real Shot
schema, which is a separate, larger piece of work left for its own pass.
"""
...
def _seed_cinematography(scene, shot_id, episode):
    """CLOSES A REAL, PRE-EXISTING GAP (found 2026-07-22, chasing the new keyframe spend-
    token seal through this exact fixture): keyframe_shot has required an APPROVED
    Cinematography Direction since 2026-07-19's department-gate hardening — this file never
    seeded one, so every keyframe_shot call in this suite has been refusing on
    DepartmentNotApproved (never reaching the generation-safety mechanics this file exists
    to test) for as long as that hard gate has existed, independent of tonight's own seal
    work. Writes a real, correctly-signed approval directly (this file has no cb_llm/
    cb_departments mocking infrastructure at all, unlike test_cb_render_department_gate.py's
    heavier fixture — adding that here would be a second, larger change) using the SAME
    _department_context_for_freshness/_department_signature functions department_freshness
    itself calls, so the approval reads as genuinely CURRENT rather than a hand-faked hash
    that happens to match today by luck. Must be called again after every _write_package
    (which rewrites continuityLedger from scratch, wiping any earlier approval)."""
    pkg, path = E.canonical_package_path(scene, episode), None
    pkg_path = E.canonical_package_path(scene, episode)
    d = json.loads(pkg_path.read_text())
    context = R._department_context_for_freshness(d, scene, "cinematography", shot_id, episode)
    sig = R._department_signature(context)
    led = R._ledger(d, shot_id)
    led.setdefault("departmentWork", {})["cinematography"] = {
        "approved": {"output": {"shotId": shot_id, "doesItLand": "t",
                                  "providerPrompt": f"a real cinematography prompt for {shot_id}"},
                     "sourceHash": sig, "outcome": "approved", "reviewedBy": "test",
                     "decisionAt": "t"},
        "candidate": None, "history": []}
    pkg_path.write_text(json.dumps(d, indent=1))


def _fire_keyframe(scene, shot_id, episode):
    """Drives cb_render.keyframe_shot's real two-phase disclose-then-confirm seal
    (2026-07-22, Julian's directive closing the exact gap this route had — see
    keyframe_shot's/_keyframe_binding_hash's own docstrings) in one call, for fixture code
    that isn't itself testing the disclosure step. Never used by cb_render.py or any
    Studio route — the real split stays the only path a human ever exercises."""
    try:
        R.keyframe_shot(scene, shot_id, episode)
    except R.Refused:
        pass
    ...
    if not auth:
        raise AssertionError(f"keyframe_shot({shot_id!r}) never issued a spend token")
    return R.keyframe_shot(scene, shot_id, episode, spend_token=auth["token"])
...
def _write_package(root, scene, episode, shot_ids=("S1.SH1", "S1.SH6"), revision=1, extra_ledgers=None):
    ...
    # FIXED 2026-07-22 (found chasing the new keyframe spend-token seal through this exact
    # fixture): "irrelevant-now" was a placeholder that never actually matched
    # _require_current_lineage's own real live-storyboard-md5 comparison — this fixture never
    # exercised past that hard refusal before now (an untracked, pre-existing gap, unrelated
    # to the seal itself). Computes the REAL live md5 so keyframe_shot can actually reach its
    # own generation step, the way this file's own test names ("produces ONE candidate")
    # already assume it does.
    pkg = {"episode": episode, "sceneNumber": str(scene), "revision": revision,
           "validation": {"passed": True},
           "sourceStoryboard": {"path": "n/a", "md5": R._current_storyboard_md5(scene, episode),
...
def main():
    ...
    try:
        ...
        _seed_cinematography(scene, "S1.SH1", episode)
        _fire_keyframe(scene, "S1.SH1", episode)
